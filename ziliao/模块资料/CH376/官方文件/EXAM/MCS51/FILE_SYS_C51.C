/* CH376芯片 文件系统层 V1.3 */
/* 本文件与FILE_SYS.C功能相同,只是针对MCS51单片机进行速度优化,优化包括内部RAM/外部RAM等多种指针PUINT8I/PUINT8X存取、大小端转换和移位等 */
/* 提供文件系统常用子程序,提供命令打包 */
...
UINT8	ExchgBuf[4];	/* 临时变量,用于大小端转换和移位 */

UINT8	CH376ReadBlockI( PUINT8I buf )  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
{
	UINT8	s, l;
...
UINT8	CH376ReadBlockX( PUINT8X buf )  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
{
	UINT8	s, l;
	xWriteCH376Cmd( CMD01_RD_USB_DATA0 );
	s = l = xReadCH376Data( );  /* 长度 */
...
{
	if ( *(PUINT8)&buf ) return( CH376ReadBlockX( buf ) );  /* 外部RAM */
	else return( CH376ReadBlockI( buf ) );  /* 内部RAM */
}

UINT8	CH376WriteReqBlockI( PUINT8I buf )  /* 向内部指定缓冲区写入请求的数据块,返回长度 */
{
	UINT8	s, l;
...
UINT8	CH376WriteReqBlockX( PUINT8X buf )  /* 向内部指定缓冲区写入请求的数据块,返回长度 */
{
	UINT8	s, l;
	xWriteCH376Cmd( CMD01_WR_REQ_DATA );
	s = l = xReadCH376Data( );  /* 长度 */
...
{
	if ( *(PUINT8)&buf ) return( CH376WriteReqBlockX( buf ) );  /* 外部RAM */
	else return( CH376WriteReqBlockI( buf ) );  /* 内部RAM */
}

void	CH376WriteHostBlockX( PUINT8X buf, UINT8 len )  /* 向USB主机端点的发送缓冲区写入数据块 */
{
...
{
	ExchgBuf[3] = xReadCH376Data( );
	ExchgBuf[2] = xReadCH376Data( );
	ExchgBuf[1] = xReadCH376Data( );
	ExchgBuf[0] = xReadCH376Data( );
	xEndCH376Cmd( );
	return( *(PUINT32)ExchgBuf );
}
...
//			s = CH376ReadBlock( buf );  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
			if ( *(PUINT8)&buf ) s = CH376ReadBlockX( buf );  /* 外部RAM */
			else s = CH376ReadBlockI( buf );  /* 内部RAM */
...
//			s = CH376WriteReqBlock( buf );  /* 向内部指定缓冲区写入请求的数据块,返回长度 */
			if ( *(PUINT8)&buf ) s = CH376WriteReqBlockX( buf );  /* 外部RAM */
			else s = CH376WriteReqBlockI( buf );  /* 内部RAM */
...
UINT8	CH376DiskQuery( PUINT32 DiskFre )  /* 查询磁盘剩余空间信息,扇区数 */
{
	...
	if ( s == USB_INT_SUCCESS ) {  /* 参考CH376INC.H文件中CH376_CMD_DATA结构的DiskQuery */
		...
		xReadCH376Data( );
		xReadCH376Data( );
		ExchgBuf[3] = xReadCH376Data( );  /* CH376_CMD_DATA.DiskQuery.mFreeSector */
		ExchgBuf[2] = xReadCH376Data( );
		ExchgBuf[1] = xReadCH376Data( );
		ExchgBuf[0] = xReadCH376Data( );
		*DiskFre = *(PUINT32)ExchgBuf;
...
UINT8	CH376DiskReadSec( PUINT8X buf, UINT32 iLbaStart, UINT8 iSectorCount )  /* 从U盘读取多个扇区的数据块到缓冲区,不支持SD卡 */
/* iLbaStart 是准备读取的线性起始扇区号, iSectorCount 是准备读取的扇区数 */
{
	UINT8	s, err;
	UINT16	mBlockCount;
	*(PUINT32)ExchgBuf = iLbaStart;
	for ( err = 0; err != 3; ++ err ) {  /* 出错重试 */
		xWriteCH376Cmd( CMD5H_DISK_READ );  /* 从USB存储器读扇区 */
		xWriteCH376Data( ExchgBuf[3] );  /* LBA的最低8位 */
		xWriteCH376Data( ExchgBuf[2] );
		xWriteCH376Data( ExchgBuf[1] );
		xWriteCH376Data( ExchgBuf[0] );
		...
		for ( mBlockCount = iSectorCount * DEF_SECTOR_SIZE / CH376_DAT_BLOCK_LEN; mBlockCount != 0; -- mBlockCount ) {  /* 数据块计数 */
			...
			if ( s == USB_INT_DISK_READ ) {  /* USB存储器读数据块,请求数据读出 */
				s = CH376ReadBlockX( buf );  /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
...
UINT8	CH376DiskWriteSec( PUINT8X buf, UINT32 iLbaStart, UINT8 iSectorCount )  /* 将缓冲区中的多个扇区的数据块写入U盘,不支持SD卡 */
	...
	for ( err = 0; err != 3; ++ err ) {  /* 出错重试 */
		...
		for ( mBlockCount = iSectorCount * DEF_SECTOR_SIZE / CH376_DAT_BLOCK_LEN; mBlockCount != 0; -- mBlockCount ) {  /* 数据块计数 */
			...
			if ( s == USB_INT_DISK_WRITE ) {  /* USB存储器写数据块,请求数据写入 */
				CH376WriteHostBlockX( buf, CH376_DAT_BLOCK_LEN );  /* 向USB主机端点的发送缓冲区写入数据块 */
...
UINT8	CH376SecRead( PUINT8X buf, UINT8 ReqCount, PUINT8 RealCount )  /* 以扇区为单位从当前位置读取数据块,不支持SD卡 */
{
	UINT8	s;
	UINT8	cnt;
	UINT32	StaSec;
	...
	do {
		...
		if ( cnt == 0x41 ) {
			...
			ExchgBuf[3] = xReadCH376Data( );
			ExchgBuf[2] = xReadCH376Data( );
			cnt = ExchgBuf[1] = xReadCH376Data( );
			fsz = *(PUINT32)ExchgBuf;
			xWriteCH376Cmd( CMD14_READ_VAR32 );
...
UINT8	CH376SecWrite( PUINT8X buf, UINT8 ReqCount, PUINT8 RealCount )  /* 以扇区为单位在当前位置写入数据块,不支持SD卡 */
{
	UINT8	s;
	UINT8	cnt;
	UINT32	StaSec;
...
UINT8	CH376GetLongName( PUINT8 PathName, PUINT8X LongName )  /* 由短文件名或者目录(文件夹)名获得相应的长文件名 */
...
UINT8	CH376CreateLongName( PUINT8 PathName, PUINT8X LongName )  /* 新建具有长文件名的文件,关闭文件后返回,LongName输入路径必须在RAM中 */
