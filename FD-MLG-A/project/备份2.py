...
class ExperimentApp:
        ...
        self.data = {
            "平衡法": {
                "params": {
                    ...
                    "g": "9.794",        # 重力加速度 m/s²
                    "rho1": "981",       # 油滴密度 kg/m³
                    "rho2": "1.188",     # 空气密度 kg/m³
                    "eta": "1.832×10⁻⁵", # 空气粘滞系数 kg/(m·s)
                    "b": "0.00823"       # 修正常数 N/m
                },
                "table": [],             # 表格数据，每行是一个列表
                "chart_data": {
    ...
    def create_charge_display_boxes(self):
        ...
        self.charge_frame.place(x=10+440+20+25-100, y=320-200-30, width=70, height=180)
    ...
    def create_frames(self):
        ...
        self.root.grid_columnconfigure(1, weight=5)  # 右列
    ...
    def calculate_droplet_radius(self, base_radius, current_focus, min_focus_value):
        ...
        if max_distance == 0:
            ...
            # 使用指数函数或幂函数创建非线性变化
            scale = 0.5 + 2.5 * (distance_ratio ** 0.7)
        
        # 计算最终半径
        final_radius = int(base_radius * scale)
        
        # 确保半径在合理范围内（至少1像素，最大不超过base_radius*3）
        final_radius = max(1, min(final_radius, int(base_radius * 3)))
    ...
    def initialize_display(self):
        """初始化显示屏"""
        # 创建显示屏区域但不显示内容
        self.display_canvas = tk.Canvas(self.frame_top_left, width=400, height=330, bg='black')
        self.display_canvas.pack(padx=10, pady=10)
    ...
    def show_balance_interface_canvas(self):
        ...
        settings_text = canvas.create_text(330, 270+20, text="参数设置", 
    ...
    def switch_display_interface(self, interface_name):
        ...
        if interface_name in self.images:
            img = self.images[interface_name].resize((400, 334), Image.Resampling.LANCZOS)
    ...
    def show_dynamic_interface_canvas(self):
        ...
        dynamic_settings_button = canvas.create_text(330, 270+20, text="参数设置", 
                                                    font=('Arial', 10), anchor='w')
    ...
    def get_unified_parameters(self, method="balance"):
        ...
        method_key = "平衡法" if method in ["balance", "平衡法"] else "动态法"
        params = self.data[method_key]["params"].copy()
        
        # ========== 统一计算派生参数（确保一致） ==========
        R = 287.05  # 空气气体常数 J/(kg·K)
        
        # 转换参数类型
        l = float(params.get('l', '2.0')) / 1000  # mm转m
        T = float(params.get('T', '297.15'))
        P = float(params.get('P', '101325'))
        g = float(params.get('g', '9.794'))
        rho1 = float(params.get('rho1', '981'))
        b = float(params.get('b', '0.00823'))
        ...
        print(f"\n【统一参数计算结果 - {method_key}】")
        print(f"运动距离 l = {l*1000:.1f} mm = {l:.6f} m")
        print(f"温度 T = {T:.2f} K")
        print(f"大气压强 P = {P} Pa")
        print(f"重力加速度 g = {g} m/s²")
        print(f"油滴密度 ρ1 = {rho1} kg/m³")
        print(f"空气密度 ρ2 = {rho2:.6f} kg/m³")
        print(f"空气粘滞系数 η = {eta:.6e} Pa·s")
        print(f"极板间距 d = {d} m")
        print(f"修正常数 b = {b} N/m")
    ...
    def create_data_record_area(self):
        """创建数据记录区域 - 改进：二级选项卡系统"""
        # 主选项卡框架 - 实验方法选择
        method_tab_frame = tk.Frame(self.frame_bottom_right)
        method_tab_frame.pack(fill=tk.X, padx=10, pady=(5, 0))
        
        # 实验方法选择
        tk.Label(method_tab_frame, text="选择实验方法:", font=('Arial', 10)).pack(side=tk.LEFT, padx=5)
        
        # 创建选项变量
        self.method_var = tk.StringVar(value="平衡法")
        
        # 创建单选按钮
        balance_radio = tk.Radiobutton(method_tab_frame, text="平衡法", variable=self.method_var, 
                                    value="平衡法", command=self.update_method_content)
        balance_radio.pack(side=tk.LEFT, padx=10)
        
        dynamic_radio = tk.Radiobutton(method_tab_frame, text="动态法", variable=self.method_var, 
                                    value="动态法", command=self.update_method_content)
        dynamic_radio.pack(side=tk.LEFT, padx=10)
        
        # 二级选项卡框架 - 内容选择
        self.content_tab_frame = tk.Frame(self.frame_bottom_right, bg='#f0f0f0')
        self.content_tab_frame.pack(fill=tk.X, padx=10, pady=(10, 5))
        
        # 内容类型变量
        self.content_type_var = tk.StringVar(value="实验参数")
        
        # 内容类型选择按钮
        content_types = ["实验参数", "数据表格", "数据曲线"]
        content_buttons = []
        
        for i, content_type in enumerate(content_types):
            btn = tk.Radiobutton(
                self.content_tab_frame, 
                text=content_type, 
                variable=self.content_type_var, 
                value=content_type,
                command=self.update_content_type,
                font=('Arial', 10),
                indicatoron=0,  # 显示为按钮形式而不是单选按钮圆圈
                width=12,
                padx=10,
                pady=5,
                bg='#e0e0e0',
        ...
        # 初始显示平衡法内容
        self.update_method_content()
    
    def update_method_content(self):
        """更新实验方法选择后的内容"""
        old_method = getattr(self, '_current_method', None)
        new_method = self.method_var.get()
        
        print(f"切换实验方法: 从 {old_method} 到 {new_method}")
        
        # 在切换实验方法前保存当前表格数据
        if old_method == "平衡法":
            try:
                # 检查平衡法表格控件是否存在且可用
                if hasattr(self, 'balance_tree') and self.balance_tree.winfo_exists():
                    self.save_balance_table_data()
                    print("✅ 已保存平衡法表格数据")
                else:
                    print("⚠️ 平衡法表格控件不存在或已被销毁")
            except Exception as e:
            ...
            try:
                # 检查动态法表格控件是否存在且可用
                if hasattr(self, 'dynamic_tree') and self.dynamic_tree.winfo_exists():
                    self.save_dynamic_table_data()
                    print("✅ 已保存动态法表格数据")
                else:
                    print("⚠️ 动态法表格控件不存在或已被销毁")
            except Exception as e:
        ...
        # 重置内容类型到"实验参数"
        self.content_type_var.set("实验参数")
        
        # 更新内容
        self.update_content_type()

    # 在 update_content_type 方法中，销毁控件时更加小心
    def update_content_type(self):
        """更新内容类型选择后的显示"""
        method = self.method_var.get()
        content_type = self.content_type_var.get()
        
        print(f"显示内容: {method} - {content_type}")
        
        # 安全地清除当前内容
        for widget in self.content_frame.winfo_children():
            try:
                if widget.winfo_exists():  # 检查控件是否仍然存在
                    widget.destroy()
            except Exception as e:
                print(f"销毁控件时出错: {e}")
                # 尝试其他方式销毁
        ...
        # 保存当前内容类型
        self._current_content = content_type
        
        # 根据方法和内容类型显示相应内容
        if method == "平衡法":
            if content_type == "实验参数":
                self.show_balance_params()
            elif content_type == "数据表格":
                self.show_balance_table()
            elif content_type == "数据曲线":
                self.show_balance_chart()
        else:  # 动态法
            if content_type == "实验参数":
                self.show_dynamic_params()
            elif content_type == "数据表格":
                self.show_dynamic_table()
            elif content_type == "数据曲线":
                self.show_dynamic_chart()

    def show_balance_params(self):
        """显示平衡法参数界面"""
        # 创建带滚动条的主容器
        ...
        # 从内存数据加载参数
        params = self.data["平衡法"]["params"]
        
        # 创建变量并绑定到内存数据
        self.balance_l_var = tk.StringVar(value=params["l"])
        self.balance_T_var = tk.StringVar(value=params["T"])
        self.balance_P_var = tk.StringVar(value=params["P"])
        self.balance_g_var = tk.StringVar(value=params["g"])
        self.balance_rho1_var = tk.StringVar(value=params["rho1"])
        self.balance_rho2_var = tk.StringVar(value=self.calculate_air_density("平衡法"))
        self.balance_eta_var = tk.StringVar(value=self.calculate_eta_display("平衡法"))
        self.balance_b_var = tk.StringVar(value=params["b"])
        ...
        # 添加参数自动更新到内存的功能
        def update_param_in_memory(var_name, value):
            """更新内存中的参数"""
            if value:
                self.data["平衡法"]["params"][var_name] = value
                print(f"参数 {var_name} 更新为: {value}")
        
        def save_params_on_change(var_name, *args):
            """参数变化时自动更新内存"""
            value = ""
            if var_name == "l":
                value = self.balance_l_var.get()
            elif var_name == "T":
                value = self.balance_T_var.get()
            elif var_name == "P":
                value = self.balance_P_var.get()
            elif var_name == "g":
                value = self.balance_g_var.get()
            elif var_name == "rho1":
                value = self.balance_rho1_var.get()
            elif var_name == "b":
                value = self.balance_b_var.get()
            
            if value:
                update_param_in_memory(var_name, value)
        
        # 绑定参数变化事件
        self.balance_l_var.trace('w', lambda *args: save_params_on_change("l", *args))
        self.balance_T_var.trace('w', lambda *args: save_params_on_change("T", *args))
        self.balance_P_var.trace('w', lambda *args: save_params_on_change("P", *args))
        self.balance_g_var.trace('w', lambda *args: save_params_on_change("g", *args))
        self.balance_rho1_var.trace('w', lambda *args: save_params_on_change("rho1", *args))
        self.balance_b_var.trace('w', lambda *args: save_params_on_change("b", *args))
        
        # 添加自动更新空气密度和粘滞系数的功能
        def update_derived_params(*args):
            """更新派生参数（空气密度和粘滞系数）"""
            try:
                # 获取当前参数
                T_val = float(self.balance_T_var.get())
                P_val = float(self.balance_P_var.get())
                
                # 计算空气密度
                R = 287.05
                rho2 = P_val / (R * T_val)
                self.balance_rho2_var.set(f"{rho2:.3f}")
                
                # 计算空气粘滞系数
                T0 = 273.15
                eta0 = 1.716e-5
                S = 110.4
                eta = eta0 * (T_val/T0)**1.5 * (T0 + S) / (T_val + S)
                eta_display = eta / 1e-5
                self.balance_eta_var.set(f"{eta_display:.3f}×10⁻⁵")
                
                print(f"派生参数更新: ρ2={rho2:.3f}, η={eta:.3e}")
            except:
                pass
        
        # 当温度或压强变化时更新派生参数
        self.balance_T_var.trace('w', update_derived_params)
        self.balance_P_var.trace('w', update_derived_params)
    ...
    def calculate_air_density(self, method):
        """计算空气密度"""
        params = self.data[method]["params"]
        try:
            T = float(params["T"])
            P = float(params["P"])
            R = 287.05
    ...
    def calculate_eta_display(self, method):
        """计算空气粘滞系数显示值"""
        params = self.data[method]["params"]
        try:
            T = float(params["T"])
    ...
    def show_balance_table(self):
        ...
        # 1. 创建表格框架（固定高度，使用滚动条）
        table_frame = tk.LabelFrame(main_container, text="数据记录表", font=('Arial', 10, 'bold'))
        table_frame.pack(fill=tk.BOTH, expand=True)
        
        # 设置表格区域固定高度
        table_container = tk.Frame(table_frame, height=200)
        ...
        # 从内存数据加载表格
        table_data = self.data["平衡法"]["table"]
        # 创建Treeview
        self.balance_tree = ttk.Treeview(table_container, columns=columns, show="headings", height=20)
        ...
        # 创建垂直滚动条
        tree_v_scrollbar = tk.Scrollbar(table_container, orient=tk.VERTICAL)
        tree_v_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # 创建水平滚动条
        tree_h_scrollbar = tk.Scrollbar(table_container, orient=tk.HORIZONTAL)
        ...
        # 按钮
        buttons = [
            ("计算", calculate_and_save),
            ("删除选中行", delete_and_save),
            ("清空数据", clear_and_save),
    ...
    def on_balance_table_keyrelease(self, event):
        ...
        def delayed_save():
            self.update_balance_table_in_memory()
            ...
            # 如果最后一行前4列有数据，添加新行
            if values and any(v.strip() for v in values[:4]):
                self.balance_tree.insert("", "end", values=("", "", "", "", "", "", "", "", ""))
        
        self._save_timer = self.root.after(500, delayed_save)  # 500ms后保存

    def update_balance_table_in_memory(self):
        """更新内存中的平衡法表格数据"""
        table_data = []
        for item in self.balance_tree.get_children():
        ...
        self.data["平衡法"]["table"] = table_data
        print(f"平衡法表格数据已更新到内存，共{len(table_data)}行")
    ...
    def show_balance_chart(self):
        ...
        # 创建图表区域
        chart_frame = tk.LabelFrame(main_container, text="电荷-电子数关系图", font=('Arial', 10, 'bold'))
        chart_frame.pack(fill=tk.BOTH, expand=True)
        
        chart_container = tk.Frame(chart_frame, height=200)
        chart_container.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        chart_container.pack_propagate(False)
        
        self.balance_chart_canvas = tk.Canvas(chart_container, bg='white')
        self.balance_chart_canvas.pack(fill=tk.BOTH, expand=True)
        
        # 从内存中获取图表数据
        n_values = self.data["平衡法"]["chart_data"]["n_values"]
        q_values = self.data["平衡法"]["chart_data"]["q_values"]
        
        # 如果内存中有数据，直接绘制
        if n_values and q_values:
        ...
        def refresh_chart():
            """刷新图表"""
            if hasattr(self, 'balance_chart_canvas'):
                self.update_balance_chart_display()
        
        # tk.Button(button_frame, text="重新绘制图表", 
        #         command=refresh_chart,
        #         font=('Arial', 9), width=12).pack(side=tk.LEFT, padx=5)
        
        # tk.Button(button_frame, text="导出图表", 
        #         command=self.export_balance_chart,
        #         font=('Arial', 9), width=12).pack(side=tk.LEFT, padx=5)
    ...
    def calculate_balance_data(self):
        ...
        try:
            ...
            if hasattr(self, 'balance_l_var'):
                ...
                self.balance_b_var.set(f"{unified_params['b']:.5f}")
        ...
        for item in self.balance_tree.get_children():
            ...
            if len(values) >= 4 and all(values[:4]):
                try:
                    ...
                    print(f"\n=== 数据记录区域计算 - 使用统一参数 ===")
                    print(f"行数据: U={U}V, tg1={tg1}s, tg2={tg2}s, tg3={tg3}s")
                    
                    # 计算平均下落时间（保留2位小数）
                    tg_avg = round((tg1 + tg2 + tg3) / 3, 2)
                    values[4] = f"{tg_avg:.2f}"
        ...
        # 4. 如果当前显示的是图表，则更新图表
        if hasattr(self, '_current_content') and self._current_content == "数据曲线":
            if hasattr(self, 'balance_chart_canvas'):
                print("当前显示图表，更新图表显示")
                # 从内存获取最新的图表数据并绘制
                self.update_balance_chart_display()
    ...
    def plot_balance_chart(self):
        """绘制平衡法图表数据点"""
        if not hasattr(self, 'balance_chart_canvas'):
            print("错误：图表画布不存在")
            return
        ...
        # 绘制坐标轴和数据点
        self.draw_balance_chart_axes(n_min, n_max, q_min, q_max)
        ...
        n_min_display = info['n_min']
        n_max_display = info['n_max']
        q_min_display = info['q_min']
        q_max_display = info['q_max']
    ...
    def draw_balance_chart_axes(self, n_min=None, n_max=None, q_min=None, q_max=None):
        ...
        # 边距设置 - 减小边距以扩大绘图区域
        margin_top = 30  # 从40减小到30
        margin_bottom = 60  # 从70减小到60
        margin_left = 50  # 从60减小到50
        margin_right = 20
        ...
        # Y轴标签 - 位置调整
        canvas.create_text(margin_left - 35, margin_top + graph_height/2, 
                        text="q/×10⁻¹⁹C", 
                        angle=90, 
        ...
        # X轴标签 - 位置调整
        canvas.create_text(margin_left + graph_width/2, margin_top + graph_height + 35, 
                        text="电子数 n", 
                        font=('Arial', font_size, 'bold'),
                        fill='darkblue')
        
        # 图表标题 - 减小字体大小
        canvas.create_text(width//2, 15,  # 位置上移
                        text="电荷-电子数关系图", 
                        font=('Arial', 11, 'bold'),  # 从12减小到11
                        fill='darkred')
        ...
        # 添加数据边距
        n_range = n_max_display - n_min_display
        q_range = q_max_display - q_min_display
        
        # 如果数据范围太小，扩大显示范围
        if n_range < 2:
            n_min_display = max(0, n_min_display - 1)
            n_max_display = n_max_display + 1
        
        if q_range < 2:
            q_min_display = max(0, q_min_display - 1)
            q_max_display = q_max_display + 1
        ...
        for i in range(y_ticks + 1):
            ...
            # 刻度线 - 缩短长度
            canvas.create_line(margin_left - 5, y, margin_left, y, width=1, fill='black')  # 从8减小到5
            ...
            canvas.create_text(margin_left - 8, y,  # 位置调整
                            text=label, 
                            anchor='e', 
                            font=('Arial', font_size-2),  # 更小的字体
                            fill='darkgreen')
        ...
        for value in tick_values:
            ...
            canvas.create_line(x, margin_top + graph_height, 
                            x, margin_top + graph_height + 5,  # 从8减小到5
                            width=1, fill='black')
            
            # 刻度标签 - 使用更小的字体
            canvas.create_text(x, margin_top + graph_height + 10,  # 位置调整
    ...
    def import_balance_data(self):
        ...
        try:
            ...
            params_loaded = False
            print("开始导入参数...")
            params_loaded = self.load_balance_params_from_excel(file_path)  # 使用正确的方法
            ...
            if '数据记录表' in sheet_names:
                ...
                imported_rows = self.load_balance_table_data_from_excel(file_path, '数据记录表')
                if imported_rows > 0:
                    data_loaded = True
            else:
                print("没有找到'数据记录表'sheet，尝试从其他sheet导入...")
                # 尝试从所有sheet中查找数据表
                for sheet_name in sheet_names:
                    if '数据' in sheet_name or '表' in sheet_name:
                        print(f"尝试从sheet '{sheet_name}' 导入表格数据...")
                        imported_rows = self.load_balance_table_data_from_excel(file_path, sheet_name)
                        if imported_rows > 0:
                            data_loaded = True
                            break
            ...
            if data_loaded:
                ...
                if current_rows < 20:
                    for _ in range(20 - current_rows):
                        # 获取Treeview的实际列数
                        if hasattr(self.balance_tree, 'columns'):
                            num_columns = len(self.balance_tree['columns'])
                        else:
                            num_columns = 9  # 默认9列
                        self.balance_tree.insert("", "end", values=[""] * num_columns)
    ...
    def load_balance_table_data_from_excel(self, file_path, sheet_name):
        """从Excel加载平衡法表格数据"""
        try:
            print(f"加载表格数据: {file_path} - {sheet_name}")
            
            # 读取Excel文件
            df = pd.read_excel(file_path, sheet_name=sheet_name, header=None)
            data_list = df.values.tolist()
            
            print(f"读取到 {len(data_list)} 行数据")
            ...
            for i, row in enumerate(data_list):
                ...
                # 将行转换为字符串列表
                row_str = [str(cell) if cell is not None and not pd.isna(cell) else "" for cell in row[:5]]
                
                # 检查是否包含表头关键词
                header_keywords = ['电压', '下落时间', '平均值', '下落速度', '油滴半径', '油滴带电量', '带电子数']
                found_count = sum(1 for keyword in header_keywords if any(keyword in cell for cell in row_str))
                
                if found_count >= 3:  # 找到至少3个关键词
                    table_start_row = i
                    table_headers_found = True
                    print(f"找到表头在第 {i} 行: {row_str}")
                    break
            
            # 如果没有找到明确的表头，尝试找数据开始位置
            ...
            # 从开始行读取数据
            imported_rows = 0
            for i in range(table_start_row, len(data_list)):
                row = data_list[i]
                ...
                for cell in row:
                    if cell is None or pd.isna(cell):
                        ...
                        try:
                            ...
                            if 'e' in cell_str.lower() or '×10' in cell_str:
                                values.append(cell_str)
                            elif cell_str.count('.') > 0 and len(cell_str.split('.')[1]) > 3:
                                values.append(f"{cell_float:.3f}")
                            else:
                                values.append(cell_str)
                        except:
                            values.append(cell_str)
                ...
                # 只导入至少有前4个值的行
                if len(values) >= 4 and any(values[:4]):
                    # 如果少于9列，补齐到9列
                    if len(values) < 9:
                    ...
                    # 打印导入的行数据（调试用）
                    if imported_rows <= 5:  # 只打印前5行
                        print(f"导入第{imported_rows}行: {values[:4]}")
            
            print(f"成功导入 {imported_rows} 行数据")
    ...
    def on_dynamic_table_keyrelease(self,event):
        """动态法表格键盘释放事件 - 只在输入时保存"""
        # 延迟保存，避免频繁操作
        if hasattr(self, '_dynamic_save_timer'):
            self.root.after_cancel(self._dynamic_save_timer)
        
        def delayed_save():
            self.update_dynamic_table_in_memory()
            ...
            # 如果最后一行前几列有数据，添加新行
            if values and any(v.strip() for v in values[:5]):  # 检查前5列是否有数据
                self.dynamic_tree.insert("", "end", values=("", "", "", "", "", "", "", "", "", "", ""))
        
        self._dynamic_save_timer = self.root.after(500, delayed_save)  # 500ms后保存

    def update_dynamic_table_in_memory(self):
        """更新内存中的动态法表格数据"""
        table_data = []
        ...
        self.data["动态法"]["table"] = table_data
        print(f"动态法表格数据已更新到内存，共{len(table_data)}行")
    ...
    def save_balance_table_data(self):
        ...
        self.data["平衡法"]["table"] = table_data
        print(f"📊 平衡法表格保存到内存: {len(table_data)} 行")
    
        # 更新图表数据
        self.update_balance_chart_data_from_table(table_data)

    def update_balance_chart_data_from_table(self, table_data):
        """从表格数据更新图表数据"""
        n_values = []
        q_values = []
        
        for row in table_data:
            if len(row) >= 9 and row[7] and row[8]:  # q列和n列
                try:
                    q = float(row[7])  # 已经是×10⁻¹⁹单位
                    n = int(row[8])
                    n_values.append(n)
                    q_values.append(q)
        ...
        self.data["平衡法"]["chart_data"]["n_values"] = n_values
        self.data["平衡法"]["chart_data"]["q_values"] = q_values
        print(f"📈 图表数据已更新: n={n_values}, q={q_values}")

    
    def save_dynamic_table_data(self):
        """保存动态法表格数据到内存"""
        if not hasattr(self, 'dynamic_tree'):
            return
        ...
        self.data["动态法"]["table"] = table_data
        print(f"📊 动态法表格保存到内存: {len(table_data)} 行")

    def update_dynamic_chart_data_from_table(self, table_data):
        """从表格数据更新动态法图表数据"""
        n_values = []
        q_values = []
        
        # 动态法表格列定义：可能需要根据实际列调整索引
        # 假设列：0:平衡电压,1:下落时间,2:上升电压,3:上升时间,4:平均下落时间,5:平均上升时间,6:下落速度,7:上升速度,8:油滴半径,9:油滴带电量,10:带电子数
        for row in table_data:
            if len(row) >= 11 and row[9] and row[10]:  # 假设q在第9列，n在第10列
                try:
                    q = float(row[9])  # 已经是×10⁻¹⁹单位
                    n = int(row[10])
                    n_values.append(n)
                    q_values.append(q)
                except (ValueError, TypeError):
        ...
        self.data["动态法"]["chart_data"]["n_values"] = n_values
        self.data["动态法"]["chart_data"]["q_values"] = q_values
        print(f"📈 动态法图表数据已更新: n={n_values}, q={q_values}")

    def show_dynamic_params(self):
        """显示动态法参数界面"""
        # 创建带滚动条的主容器
        main_container = tk.Frame(self.content_frame)
        main_container.pack(fill=tk.BOTH, expand=True)
        ...
        # 从内存数据加载参数
        params = self.data["动态法"]["params"]
        
        # 创建变量并绑定到内存数据
        self.dynamic_l_var = tk.StringVar(value=params["l"])
        self.dynamic_T_var = tk.StringVar(value=params["T"])
        self.dynamic_P_var = tk.StringVar(value=params["P"])
        self.dynamic_g_var = tk.StringVar(value=params["g"])
        self.dynamic_rho1_var = tk.StringVar(value=params["rho1"])
        self.dynamic_rho2_var = tk.StringVar(value=self.calculate_air_density("动态法"))
        self.dynamic_eta_var = tk.StringVar(value=self.calculate_eta_display("动态法"))
        self.dynamic_b_var = tk.StringVar(value=params["b"])
        
        # 继续创建输入框...
        l_entry = tk.Entry(param_grid, textvariable=self.dynamic_l_var, width=8, font=('Arial', 9))
        l_entry.grid(row=row, column=4, sticky='w', padx=5)
        tk.Label(param_grid, text="mm", font=('Arial', 9)).grid(row=row, column=5, sticky='w')
        ...
        row += 1
        tk.Label(param_grid, text="室温 T =", font=('Arial', 9)).grid(row=row, column=0, sticky='w', padx=(0, 5))
        T_entry = tk.Entry(param_grid, textvariable=self.dynamic_T_var, width=8, font=('Arial', 9))
        T_entry.grid(row=row, column=1, sticky='w', padx=5)
        tk.Label(param_grid, text="K", font=('Arial', 9)).grid(row=row, column=2, sticky='w')
        
        tk.Label(param_grid, text="大气压强 P =", font=('Arial', 9)).grid(row=row, column=3, sticky='w', padx=(20, 5))
        P_entry = tk.Entry(param_grid, textvariable=self.dynamic_P_var, width=8, font=('Arial', 9))
        P_entry.grid(row=row, column=4, sticky='w', padx=5)
        tk.Label(param_grid, text="Pa", font=('Arial', 9)).grid(row=row, column=5, sticky='w')
        ...
        row += 1
        tk.Label(param_grid, text="重力加速度 g =", font=('Arial', 9)).grid(row=row, column=0, sticky='w', padx=(0, 5))
        g_entry = tk.Entry(param_grid, textvariable=self.dynamic_g_var, width=8, font=('Arial', 9))
        g_entry.grid(row=row, column=1, sticky='w', padx=5)
        tk.Label(param_grid, text="m/s²", font=('Arial', 9)).grid(row=row, column=2, sticky='w')
        
        tk.Label(param_grid, text="油滴密度 ρ1 =", font=('Arial', 9)).grid(row=row, column=3, sticky='w', padx=(20, 5))
        rho1_entry = tk.Entry(param_grid, textvariable=self.dynamic_rho1_var, width=8, font=('Arial', 9))
        rho1_entry.grid(row=row, column=4, sticky='w', padx=5)
        tk.Label(param_grid, text="kg/m³", font=('Arial', 9)).grid(row=row, column=5, sticky='w')
        ...
        row += 1
        tk.Label(param_grid, text="修正常数 b =", font=('Arial', 9)).grid(row=row, column=0, sticky='w', padx=(0, 5))
        b_entry = tk.Entry(param_grid, textvariable=self.dynamic_b_var, width=8, font=('Arial', 9))
        b_entry.grid(row=row, column=1, sticky='w', padx=5)
        tk.Label(param_grid, text="N/m", font=('Arial', 9)).grid(row=row, column=2, sticky='w')
        ...
        def update_param_in_memory(var_name, value):
            """更新内存中的参数"""
            if value:
                self.data["动态法"]["params"][var_name] = value
                print(f"动态法参数 {var_name} 更新为: {value}")
        
        def save_params_on_change(var_name, *args):
            """参数变化时自动更新内存"""
            value = ""
            if var_name == "l":
                value = self.dynamic_l_var.get()
            elif var_name == "T":
                value = self.dynamic_T_var.get()
            elif var_name == "P":
                value = self.dynamic_P_var.get()
            elif var_name == "g":
                value = self.dynamic_g_var.get()
            elif var_name == "rho1":
                value = self.dynamic_rho1_var.get()
            elif var_name == "b":
                value = self.dynamic_b_var.get()
            
            if value:
                update_param_in_memory(var_name, value)
        
        # 绑定参数变化事件
        self.dynamic_l_var.trace('w', lambda *args: save_params_on_change("l", *args))
        self.dynamic_T_var.trace('w', lambda *args: save_params_on_change("T", *args))
        self.dynamic_P_var.trace('w', lambda *args: save_params_on_change("P", *args))
        self.dynamic_g_var.trace('w', lambda *args: save_params_on_change("g", *args))
        self.dynamic_rho1_var.trace('w', lambda *args: save_params_on_change("rho1", *args))
        self.dynamic_b_var.trace('w', lambda *args: save_params_on_change("b", *args))
        ...
        def update_derived_params(*args):
            ...
            try:
                # 获取当前参数
                T_val = float(self.dynamic_T_var.get())
                P_val = float(self.dynamic_P_var.get())
                ...
                print(f"动态法派生参数更新: ρ2={rho2:.3f}, η={eta:.3e}")
            except:
                pass
        
        # 当温度或压强变化时更新派生参数
        self.dynamic_T_var.trace('w', update_derived_params)
        self.dynamic_P_var.trace('w', update_derived_params)
    ...
    def show_dynamic_table(self):
        ...
        # 从内存数据加载表格
        table_data = self.data["动态法"]["table"]
        
        # 创建Treeview
        self.dynamic_tree = ttk.Treeview(table_container, columns=columns, show="headings", height=20)
        
        # 设置列宽
        col_widths = [80, 100, 100, 100, 100, 
                    100, 100, 100, 100, 
                    100+40, 100+40, 100+40, 100, 80]
        ...
        # 加载数据到表格
        if table_data:
            print(f"从内存加载动态法表格: {len(table_data)} 行")
            for row_data in table_data:
                self.dynamic_tree.insert("", "end", values=row_data)
        else:
            print("内存中没有动态法数据，添加空行")
            # 添加初始20行空行
            for _ in range(20):
                self.dynamic_tree.insert("", "end", values=("", "", "", "", "", "", "", "", "", "", "", "", "", "", ""))
    ...
    def show_dynamic_chart(self):
        ...
        self.dynamic_chart_canvas = tk.Canvas(chart_container, bg='white')
        self.dynamic_chart_canvas.pack(fill=tk.BOTH, expand=True)
        
        # 从内存中获取图表数据
        n_values = self.data["动态法"]["chart_data"]["n_values"]
        q_values = self.data["动态法"]["chart_data"]["q_values"]
    ...
    def plot_dynamic_chart(self):
        """绘制动态法图表数据点"""
        if not hasattr(self, 'dynamic_chart_canvas'):
            print("错误：动态法图表画布不存在")
            return
        ...
        # 绘制坐标轴和数据点
        self.draw_dynamic_chart_axes(n_min, n_max, q_min, q_max)
    ...
    def draw_dynamic_chart_axes(self, n_min=None, n_max=None, q_min=None, q_max=None):
        """绘制动态法图表坐标轴 - 与平衡法一致"""
         # 先检查画布是否存在且有效
        if not hasattr(self, 'dynamic_chart_canvas') or self.dynamic_chart_canvas is None:
            print("动态法图表画布不存在，跳过绘制")
            return
        ...
        for i in range(y_ticks + 1):
            y = margin_top + (graph_height / y_ticks) * (y_ticks - i)
            # 刻度线
            canvas.create_line(margin_left - 5, y, margin_left, y, width=1, fill='black')
        ...
        for value in tick_values:
            ...
            # 刻度线
            canvas.create_line(x, margin_top + graph_height, 
                            x, margin_top + graph_height + 5,
                            width=1, fill='black')
            
            # 刻度标签
            canvas.create_text(x, margin_top + graph_height + 10,
                            text=f"{int(round(value))}", 
                            anchor='n', 
                            font=('Arial', font_size-2),
                            fill='darkgreen')

    def update_dynamic_chart_display(self):
        """更新动态法图表显示"""
        if not hasattr(self, 'dynamic_chart_canvas'):
            print("❌ 动态法图表画布不存在")
            return
        ...
        try:
            ...
            if not n_values or not q_values:
                # 如果没有数据，显示提示
                self.draw_dynamic_chart_axes()
                self.dynamic_chart_canvas.create_text(
                    self.dynamic_chart_canvas.winfo_width()//2, 
                    self.dynamic_chart_canvas.winfo_height()//2, 
                    text="暂无数据，请先计算数据", 
                    font=('Arial', 10), fill='gray'
                )
                return
    ...
    def calculate_dynamic_data(self):
        ...
        if hasattr(self, '_current_content') and self._current_content == "数据曲线":
            # 确保画布存在并且是有效的
            if (hasattr(self, 'dynamic_chart_canvas') and 
                self.dynamic_chart_canvas is not None and 
                self.dynamic_chart_canvas.winfo_exists()):
                print("当前显示图表，更新图表显示")
                # 从内存获取最新的图表数据并绘制
                self.update_dynamic_chart_display()
            else:
                print("图表画布不存在或已被销毁，无法更新图表")
                # 如果画布不存在但应该显示图表，重新创建它
                if self.method_var.get() == "动态法":
                    print("重新创建动态法图表")
                    self.show_dynamic_chart()
                    self.update_dynamic_chart_display()
        else:
