  ...
  if(htim->Instance==htim3.Instance)
	...
	//	HAL_UART_Receive_IT(&huart4, RX_buffer, 4);
		...
		if(tim3_counter_1>=1000) //10K/1000=10Hz
		{
			...
			if(osc_state==FREE)
			{
				//pt100_voltage=Convert_To_Voltage();
				
			//	temp_light_now=pt100_getTemperature();
		//		printf("T_pool.txt=\"%.1f℃\"\xff\xff\xff",temp_pool_now);
			//	temp_light_now=current_temperature;
				if(start_ctrl==1&&cursor!=T_LIGHT) printf("T_light.txt=\"%.1f℃\"\xff\xff\xff",temp_light_now);
				else printf("T_light.txt=\"%.1f℃\"\xff\xff\xff",temp_light_set);
				if(start_ctrl==1&&cursor!=T_POOL)	printf("T_pool.txt=\"%.1f℃\"\xff\xff\xff",temp_pool_now);
				else printf("T_pool.txt=\"%.1f℃\"\xff\xff\xff",temp_pool_set);
		//		DAC_renew();
				renew_data();
				renew_menu();
			}
	...
	{	
		...
		if(tim5_counter_1>=10)//100/10=10Hz
		{
			tim5_counter_1=0;
			if(osc_state==FREE)
			get_encoder();
	...
	if (htim->Instance == TIM4&&start_ctrl==1) 
	{
		...
		else if(start_ctrl==1)
		{
			...
			{
				tim4_counter=0;
				temp_pool_now = DS18B20_Get_Temperature()*0.0625;
			//	temp_light_now=pt100_getTemperature();
				temp_light_now = DS18B20_Get_Temperature_1()*0.0625;
			}
			if(tim4_counter_1>=100)//100/10=10Hz
			{
				tim4_counter_1=0;
