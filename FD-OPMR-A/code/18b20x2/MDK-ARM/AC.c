...
void GenerateSineTable(void) {
    for (int i = 0; i < num; i++) {
        sine_table[i] = (int16_t)(VPP/2.00*sin(2.00*M_PI*i/num));
    }
}
...
void sin_handle()
{
	...
	{
		dac_value=(dac_forward_max+dac_forward_min)/2.00+sine_table[index];
		index = (index + 1) %num;
	}
	if(current_direction==reverse)
	{
		dac_value=(dac_reverse_max+dac_reverse_min)/2.00+sine_table[index];
		index = (index + 1) %num;
	}
}
...
void triangle_handle()
{
	if(current_direction==forward)
	{
		...
		direction_counter++;
	//	if(direction_counter==num/2)delay_ms(50);
		if(direction_counter==num) 
		{
			direction_counter=0;
		//	delay_ms(50);
		}
...
void square_handle()
{
	if(current_direction==forward)
	{
		if(direction_counter<num*duty/100.00) dac_value=dac_forward_max;
	...
	{
		if(direction_counter<num*duty/100.00) dac_value=dac_reverse_max;
