...
float temp_light_set=90,temp_pool_set=60,temp_pool_now,temp_light_now,current_hori=0,current_vert=0,current_scan=50;
...
char *mag_scale[]={"10V","5V","2V","1V","500mV","200mV","100mV","50mV","20mV"};
char *light_scale[]={"10V","5V","2V","1V","500mV","200mV","100mV","50mV","20mV"};
...
void left_button()
{
	switch(cursor)
	{
		case T_LIGHT:if(temp_light_set>temp_light_min)temp_light_set-=0.1;break;
		case T_POOL:if(temp_pool_set>temp_pool_min)temp_pool_set-=0.1;break;
		case I_HORI:if(current_hori>-200)current_hori-=0.1;DAC_renew();break;
		case I_SCAN:if(current_scan>0)current_scan-=0.1;DAC_renew();break;
		case I_VERT:if(current_vert>-200)current_vert-=0.1;break;
...
void right_button()
{
	switch(cursor)
	{
		case T_LIGHT:if(temp_light_set<temp_light_max)temp_light_set+=0.1;break;
		case T_POOL:if(temp_pool_set<temp_pool_max)temp_pool_set+=0.1;break;
		case I_HORI:if(current_hori<200)current_hori+=0.1;DAC_renew();break;
...
void renew_menu_all()
{
		printf("%s.txt=\"%.1f℃\"\xff\xff\xff",txtname[T_LIGHT],temp_light_set);
		printf("%s.txt=\"%.1f℃\"\xff\xff\xff",txtname[T_POOL],temp_pool_set);
		if(start_ctrl==0) printf("%s.txt=\"开始控温\"\xff\xff\xff",txtname[CTRL]);		
		else printf("%s.txt=\"结束控温\"\xff\xff\xff",txtname[CTRL]);
		printf("%s.txt=\"%.1fmA\"\xff\xff\xff",txtname[I_HORI],current_hori);
		printf("%s.txt=\"%.1fmA\"\xff\xff\xff",txtname[I_VERT],current_vert);
		printf("%s.txt=\"%.1fmA\"\xff\xff\xff",txtname[I_SCAN],current_scan);
		printf("%s.txt=\"%s\"\xff\xff\xff",txtname[SCAN_SHAPE],scan_shape[scan_shape_now]);
		printf("%s.txt=\"%dHz\"\xff\xff\xff",txtname[FREQ],freq_scan);
		printf("%s.txt=\"%s/Div\"\xff\xff\xff",txtname[TIME_GRID],time_grid[time_grid_now]);
		printf("%s.txt=\"%s/Div\"\xff\xff\xff",txtname[MAG_SCALE],mag_scale[mag_scale_now]);
		printf("%s.txt=\"%s/Div\"\xff\xff\xff",txtname[LIGHT_SCALE],light_scale[light_scale_now]);
		printf("%s.txt=\"%s\"\xff\xff\xff",txtname[TRIG_MODE],trig_mode[trig_mode_now]);
		if(Ouhe==0) printf("%s.txt=\"%s\"\xff\xff\xff",txtname[MAG_COUPLE],"DC");
		else printf("%s.txt=\"%s\"\xff\xff\xff",txtname[MAG_COUPLE],"AC");
		if(Ouhe1==0) printf("%s.txt=\"%s\"\xff\xff\xff",txtname[LIGHT_COUPLE],"DC");
		else printf("%s.txt=\"%s\"\xff\xff\xff",txtname[LIGHT_COUPLE],"AC");
