...
void DAC_init()
{
	...
	DC_value=fabs(current_hori/1000.0000);
	AC_value=current_scan/1000.0000;
	uint16_t freq=freq_scan;	
	if(freq<20) num=100;
	else if(freq<30) num=50;
	else 
	num=30;
	dac_forward_max=(((DC_value+AC_value*0.707)*k)+Vcc)/2.00/Vcc*4096.00;
	dac_forward_min=(((DC_value-AC_value*0.707)*k)+Vcc)/2.00/Vcc*4096.00;
	dac_reverse_max=(((-DC_value+AC_value*0.707)*k)+Vcc)/2.00/Vcc*4096.00;
	dac_reverse_min=(((-DC_value-AC_value*0.707)*k)+Vcc)/2.00/Vcc*4096.00;
	...
	if(current_hori>=0) current_direction=reverse;
...
void DAC_renew()
{
	...
	num=30;
	dac_forward_max=(((DC_value+AC_value*0.707)*k)+Vcc)/2.000/Vcc*4096.000;
	dac_forward_min=(((DC_value-AC_value*0.707)*k)+Vcc)/2.000/Vcc*4096.000;
	dac_reverse_max=(((-DC_value+AC_value*0.707)*k)+Vcc)/2.000/Vcc*4096.000;
	dac_reverse_min=(((-DC_value-AC_value*0.707)*k)+Vcc)/2.000/Vcc*4096.000;
	VPP=dac_forward_max-dac_forward_min;
	switch(scan_shape_now)
	{
		case 0:
