...
class SurfaceTensionDiagram(QWidget):
    ...
    def is_ring_in_water(self):
        ...
        # 计算圆环底部位置 - 跟随传感器位置变化
        ring_bottom = int(self.height() * 0.2) - 40 + 60 + 30  # 传感器y(升高40) + 连接线高度(延长到60) + 圆环高度
        print(f"ring_bottom: {ring_bottom}")

        # 计算水面位置
        water_top = int(self.height() * (0.42 + 0.17 * (self.lift_position/100))) - 40  # 升降台y - 水高度
    ...
    def set_lift_position(self, position):
        ...
        # 计算砝码/圆环底部位置
        sensor_y = int(self.height() * 0.2) - 40  # 升高后的传感器位置
        ...
        if self.weight_count >= 3:
            ...
            if self.show_water:
                ...
                if (hasattr(self, 'has_contacted_water') and self.has_contacted_water and 
                    ...
                    elif 0 < distance <= 18:  # 显示水柱
                        # 计算梯形水柱参数
                        water_column_top = weight_y + 30
                        water_column_height = min(18, distance)
...
class SurfaceTensionApp(QMainWindow):
    def __init__(self):
        ...
        # 设置固定窗口大小（取消缩放）
        self.setFixedSize(1084, 415)  # 固定宽度1084，高度600
        self.setGeometry(0, 0, 400, 300)
        self.querying_data = False
        current_group_data=None
        self.current_group = 1
        self.current_point_index = 0
        self.background_image = "表面张力面板.png"    # 图片路径
        # 设置窗口标志，禁用最大化按钮
        ...
        self.setMinimumSize(1184, int(1184 / self.aspect_ratio))
        ...
        self.resize(1084, 415)  # 初始大小，但可以调整
        self.original_size = QSize(1084, 415)  # 存储原始大小用于缩放计算
    ...
    def init_left_area(self):
       ...
       # 左侧显示框
        ...
        self.left_display_layout.setContentsMargins(20, 0, 0, 65)  # 设置整体边距
        self.left_display_frame.setLayout(self.left_display_layout)
        
        # 左侧显示框内容
        self.display_box = QGroupBox("")
        # 设置大小
        self.display_box.setFixedSize(380, 240)  # 固定大小
        ...
        self.tension_display.setAlignment(Qt.AlignCenter)
        self.tension_display.hide()
        self.tension_display.setFixedHeight(30)  # 固定高度
        self.tension_display.setStyleSheet("""
            font-size: 16px;
            padding: 0px;
            margin: 0;
        ...
        self.measurement_layout.setContentsMargins(20, 0, 0, 20)  # 添加边距
        ...
        # 设置数据点阵的最小尺寸
        self.data_display.setMinimumSize(300, 105)  # 增大宽度和高度
        ...
        # self.nav_layout.setVerticalSpacing(5)  # 设置垂直间距为15像素
        self.nav_layout.setHorizontalSpacing(45)  # 设置水平间距
        ...
        self.nav_layout.setRowMinimumHeight(0, 60)  # 第一行最小高度60像素
        self.nav_layout.setRowMinimumHeight(1, 40)  # 第二行保持原高度

        # 添加拉伸项使按钮靠边
        self.nav_layout.setContentsMargins(0, 80, 0, 50)  # 左 20px，上 10px，右 20px，下 10px
    ...
    def init_right_area(self):
        ...
        left_layout.setContentsMargins(20, 0, 0, 0)  # 增加右margin使旋钮右移
        ...
        self.stop_btn_container.setFixedSize(120, 70)  # 容器稍大以便放置箭头
        ...
        self.stop_btn.setFixedSize(100, 70)
        ...
        stop_layout.addSpacing(28)  # 调整为-160（原为140），这会向左移动箭头
    ...
    def update_tension_display(self):
        ...
        if self.excel_data and self.data_points:
            ring_bottom = int(self.height() * 0.2) - 40 + 60 + 30  # 传感器y(升高40) + 连接线高度(延长到60) + 圆环高度
            water_top = int(self.height() * (0.42 + 0.17 * (diagram.lift_position/100))) - 40  # 升降台y - 水高度

            # 液面到excel数据结束点的距离
            distance = ring_bottom + 35 - water_top
    ...
    def collect_data(self):
       ...
       # 叠加Excel中的拉力数据
        ...
        excel_force = 0
        if self.excel_data:
            ring_bottom = self.height() * 0.2 + 60 + 30   # 传感器y(升高40) + 连接线高度(延长到60) + 圆环高度
            water_top = self.height() * (0.42 + 0.17 * (diagram.lift_position/100)) - 40  # 升降台y - 水高度
