...
class ControlWindow(QDialog):
    ...
    def init_lift_controls(self):
        ...
        # 添加缓慢下降按钮
        self.slow_down_btn = QPushButton("缓慢下降")
        self.slow_down_btn.setCheckable(True)
        self.slow_down_btn.setStyleSheet(button_style)
        self.slow_down_btn.clicked.connect(lambda: self.set_lift_direction(3))  # 3表示缓慢下降
        # 添加控件到布局
        ...
        lift_layout.addWidget(self.slow_down_btn)  # 添加缓慢下降按钮
    ...
    def set_lift_direction(self, direction):
        ...
        self.slow_down_btn.setChecked(direction == 3)
         # 设置速度
        if direction == 3:  # 缓慢下降
            self.lift_speed = 0.75
        else:  # 正常速度
            self.lift_speed = 1.5
...
class SurfaceTensionApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("FD-NST-A 液体表面张力系数测量仪")
        # 设置固定窗口大小（取消缩放）
    ...
    def add_random_variation(self, value):
        """为测量值添加0-0.5%的随机波动"""
        variation = random.uniform(-0.002, 0.002)  # -0.5%到+0.5%
