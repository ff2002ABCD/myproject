...
class SimpleImageRotator:
    ...
    def calculate_thermal_expansion(self):
        ...
        delta_length = self.sample_length * alpha * delta_temp  # 单位：mm
        
        print(f"热膨胀计算: 材料={material}, α={alpha:.2e}, ΔT={delta_temp:.1f}°C, ΔL={delta_length:.3f}mm")
    ...
    def initialize_app(self):
        ...
        # 设置初始读数（铝棒）- 随机生成（新增）
        import random
        self.reading_value = random.uniform(-0.1, 1.2)
        print(f"初始化（铝棒）: 随机读数: {self.reading_value:.3f} mm")
        self.base_reading = self.reading_value
    ...
    def setup_ui(self):
        ...
        main_frame.grid_columnconfigure(1, weight=1)  # 右列权重2
    ...
    def update_sample_label(self):
        ...
        self.sample_label.config(text=sample)
        ...
        self.theory_coeff_label.config(text=coeff_dict.get(sample, "23.1"))
    ...
    def update_temperature(self):
        ...
        # 如果正在复位流程中（temp_mode为power_off），暂停温度更新
        if self.temp_mode == "power_off":
            self.timer_running = False
            self.root.after(1000, self.update_temperature)
            return
        ...
        print(f"更新温度: 模式={self.temp_mode}, 当前={self.current_temp}, 目标={self.target_temp}")
        
        if self.temp_mode == "control":
            # 控温模式
            if self.current_temp < self.target_temp:
                self.current_temp = min(self.current_temp + 3, self.target_temp)
                print(f"升温中: 当前温度={self.current_temp}")
                self.update_display()
                # 更新红灯状态
                self.update_red_light()
            elif self.current_temp > self.target_temp:
                # 降温逻辑，但不能低于室温
                new_temp = max(self.current_temp - 0.01, max(self.target_temp, self.room_temp))
                self.current_temp = new_temp
                print(f"降温中: 当前温度={self.current_temp}")
                self.update_display()
                # 停止红灯闪烁
                self.stop_red_light_blink()
            else:
                print(f"已达到目标温度: {self.current_temp}")
            
            # 在控温模式下，温度变化时更新千分表读数（新增）
            self.update_reading_from_value()
            
        elif self.temp_mode == "power_off" or self.temp_mode == "set" or self.temp_mode == "waiting":
            # 在电源关闭或设定模式时，温度调整到室温
            if self.current_temp > self.room_temp:
                self.current_temp = max(self.current_temp - 0.01, self.room_temp)
                print(f"降温至室温: 当前温度={self.current_temp}")
                self.update_display()
            elif self.current_temp < self.room_temp:
                self.current_temp = min(self.current_temp + 1.0, self.room_temp)
                print(f"升温至室温: 当前温度={self.current_temp}")
                self.update_display()
            # 停止红灯闪烁
            self.stop_red_light_blink()
    ...
    def setup_sample_selection(self):
        ...
        # 创建样品选择变量
        self.sample_var = tk.StringVar(value="铝棒")
        ...
        for sample in samples:
            btn = tk.Radiobutton(button_frame, 
                            ...
                            width=4,
                            height=1,
                            command=lambda s=sample: self.on_sample_selected(s))  # 添加点击事件
        ...
        # 压紧量杆按钮
        tighten_btn = tk.Button(rod_frame, text="压紧量杆", 
                            bg="#4CAF50", fg="white",
                            font=("Arial", 11),
                            command=self.tighten_rod)
        tighten_btn.pack(side=tk.LEFT, padx=5)
        
        # 放松量杆按钮
        loosen_btn = tk.Button(rod_frame, text="放松量杆", 
                            bg="#f44336", fg="white",
                            font=("Arial", 11),
                            command=self.loosen_rod)
        loosen_btn.pack(side=tk.LEFT, padx=5)
    ...
    def on_sample_selected(self, sample=None):
        """样品选择事件处理"""
        if sample is None:
            sample = self.sample_var.get()  # 获取当前选择的样品
        
        import random
        # 随机生成基础读数（-0.1到1.2mm）
        self.base_reading = random.uniform(-0.1, 1.2)
        print(f"选择样品: {sample}, 基础读数: {self.base_reading:.3f} mm")
