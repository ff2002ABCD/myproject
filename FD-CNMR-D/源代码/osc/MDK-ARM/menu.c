...
void right_button()
{	
	switch(current_calibo_state)
	{
		default:break;
		case CURRENT_CALIBOING:
		{
			current_offset++;
			printf("t24.txt=\"偏移:%.0f\"\xff\xff\xff",current_offset);
		...
		switch(cursor_mode)
		{
			case 1:
			{
				if(cursor_num==0)
				{
					...
					printf("t24.txt=\"CH1电压：2\"\xFF\xFF\xFF");
				}
				else
				{
					cursor_num=0;
			...
			case 2:
			{
				if(xy_enable==1)
				{
					if(cursor_num==0&&x1<650)
					{
						...
						printf("move t16,%d,0,%d,0,0,0\xFF\xFF\xFF",x1+step,x1+step);
						printf("move t22,%d,0,%d,0,0,0\xFF\xFF\xFF",x1+step,x1+step);
						x1+=step;
					}
					else if(cursor_num==1&&x2<650)
					{
						//printf("t24.txt=\"选中：纵2\"\xFF\xFF\xFF");
						printf("move t17,%d,0,%d,0,0,0\xFF\xFF\xFF",x2+step,x2+step);
						printf("move t23,%d,0,%d,0,0,0\xFF\xFF\xFF",x2+step,x2+step);
						x2+=step;
					}
					break;
				}
	...
	switch(menu2_status)
	{
		...
		case CURSOR_MODE:
		{
			...
			switch(cursor_mode)
			{
				...
				case 1:
				{
					if(cursor_num==0) printf("t24.txt=\"CH1电压：1\"\xff\xff\xff");
					else printf("t24.txt=\"CH1电压：2\"\xff\xff\xff");
				}break;
				case 2:
				{
		...
		case CH1_SWITCH:
		{
			...
			{
				CH1_enable=0;
				printf("t39.txt=\"->CH1关\"\xff\xff\xff");
			}
			else
			{
				CH1_enable=1;
				printf("t39.txt=\"->CH1开\"\xff\xff\xff");
			}
			printf("cle 1,255\xFF\xFF\xFF");
		}break;
		case CH2_SWITCH:
		{
			...
			{
				CH2_enable=0;
				printf("t38.txt=\"->CH2关\"\xff\xff\xff");
			}
			else
			{
				CH2_enable=1;
				printf("t38.txt=\"->CH2开\"\xff\xff\xff");
			}
			printf("cle 1,255\xFF\xFF\xFF");
		}break;
	...
	switch(menu_status)
	{
		case 9:
		{
			if(Channel==0)
			{
				...
				printf("t37.txt=\"共振信号\"\xFF\xFF\xFF");
			...
			else if(Channel==1)
			{
				...
				printf("t37.txt=\"移相信号\"\xFF\xFF\xFF");
...
//						}break;
		...
		{	
			...
			printf("move t26,%d,0,%d,0,0,0\xFF\xFF\xFF",325+offset,325+offset);
		...
		case 13:
		{
			if(Channel==0)
			{
				...
				printf("move t25,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(192-1.5*(Trigger_set_offset_ch1+offset_ch1)),(int)(192-1.5*(Trigger_set_offset_ch1+offset_ch1)));
				printf("move t29,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(181-1.5*(offset_ch1)),(int)(181-1.5*(offset_ch1)));
			}
			else
			{	
				offset_ch2+=1;
				...
				printf("move t25,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(192-1.5*(Trigger_set_offset_ch2+offset_ch2)),(int)(192-1.5*(Trigger_set_offset_ch2+offset_ch2)));
				printf("move t30,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(181-1.5*(offset_ch2)),(int)(181-1.5*(offset_ch2)));
			}
		}break;
		case 14:
		{
...
void up_button()
{
		...
		switch(cursor_mode)
		{
			case 1:
			{
				if(cursor_num==0&&y1>0)
				{
					...
					printf("move t18,0,%d,0,%d,0,0\xFF\xFF\xFF",y1-step,y1-step);
					printf("move t20,0,%d,0,%d,0,0\xFF\xFF\xFF",y1-step,y1-step);
					y1-=step;
				}
				else if(cursor_num==1&&y2>0)
				{
					//printf("t24.txt=\"选中：横2\"\xFF\xFF\xFF");
					printf("move t19,0,%d,0,%d,0,0\xFF\xFF\xFF",y2-step,y2-step);
					printf("move t21,0,%d,0,%d,0,0\xFF\xFF\xFF",y2-step,y2-step);
					y2-=step;
				}
			}break;
			case 2:
	...
	{
		menu2_status--;
		if(menu2_status==0) menu2_status=10;//电流校准
		if(menu2_status==10)
		{
			if(*(__IO uint32_t*)0x08019100!=0xffffffff) menu2_status=9;
		}
		if(menu2_status==9)
		{
			if(*(__IO uint32_t*)0x08019000!=0xffffffff) menu2_status=8;//正常模式
		}
...
{	
	...
	if(menu2_status!=0)
	{
		...
		{
			if(menu2_status==9) menu2_status=10;
		}
		if(*(__IO uint32_t*)0x08019100!=0xffffffff)
		{
			if(menu2_status==10) menu2_status=11;
		}
		if(menu2_status==11) menu2_status=1;
		renew_menu2();
		return;
	}
	switch(menu_status)
...
{	
	switch(current_calibo_state)
	{
		...
		case SAVE:
		{
			current_calibo_state=CURRENT_CALIBOING;
			printf("t24.txt=\"偏移:%.0f\"\xff\xff\xff",current_offset);
		}break;
	}
	switch(caliboration_mode)
	{
		...
		case 5:
		{
			...
			if(calibo_ch==1)
			{
				offset_ch1=-96;
				printf("move t29,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(181-1.5*offset_ch1),(int)(181-1.5*(offset_ch1)));
				printf("t24.txt=\"CH1校准3/3\"\xFF\xFF\xFF");
			...
			else
			{
				offset_ch2=-96;
				printf("move t30,0,%d,0,%d,0,0\xFF\xFF\xFF",(int)(181-1.5*offset_ch2),(int)(181-1.5*(offset_ch2)));
				printf("t24.txt=\"CH2校准3/3\"\xFF\xFF\xFF");
			}
		}break;
		case 3:
		{
		...
		case 1:
		{
			...
			if(*(__IO uint32_t*)0x08019100==0xffffffff)	printf("vis t40,1\xFF\xFF\xFF");
			else printf("vis t40,0\xFF\xFF\xFF");
...
void confirm_button()
{
	switch(current_calibo_state)
	{
		case CURRENT_CALIBO_OFF:break;
		case CURRENT_CALIBOING:
		{
			current_calibo_state=SAVE;
			printf("t24.txt=\"按确定保存\"\xff\xff\xff");
		}break;
		case SAVE:
		{
			load_writedata_1();
			STMFLASH_OnlyWrite(0x08019100,Flash_WData_1,4);
			current_calibo_state=CURRENT_CALIBO_OFF;
			CH2_status=SWITCH_PARA;
			printf("vis t24,0\xff\xff\xff");
			menu2_status=0;
			menu2_off();
			renew_menu2();
	...
	if(current_calibo_state!=0) return;
	switch(caliboration_mode)
	{
		case 0:break;
		case 1:
...
void renew_menu2()
{
	printf("t44.txt=\"基线校准\"\xff\xff\xff");
	printf("t50.txt=\"光标模式\"\xff\xff\xff");
	printf("t40.txt=\"电流校准\"\xff\xff\xff");
	if(cursor_switch!=0) printf("t58.txt=\"光标开\"\xff\xff\xff");
	else printf("t58.txt=\"光标关\"\xff\xff\xff");
	if(xy_enable==0) printf("t45.txt=\"Y-T\"\xff\xff\xff");
	else printf("t45.txt=\"X-Y\"\xff\xff\xff");
	if(CH1_enable==0) printf("t39.txt=\"CH1关\"\xff\xff\xff");
	else printf("t39.txt=\"CH1开\"\xff\xff\xff");
	if(CH2_enable==0) printf("t38.txt=\"CH2关\"\xff\xff\xff");
	else printf("t38.txt=\"CH2开\"\xff\xff\xff");
	switch(Trigger_ANS)
	{
		case 0:
		{
...
//		printf("vis t56,0\xFF\xFF\xFF");
	...
	if(para_flag==0&&cursor_switch==0&&fft_enable==0)
	{
