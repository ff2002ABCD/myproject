...
{	
	float time_us;
	time_us=x*time_table[time_per_grid]/50.00/1000.00;
	return time_us;
}
...
{	
	float time_ms;
	time_ms=x*time_table[time_per_grid]/50.00/1000000.00;
	return time_ms;
}
...
void findzeropoint_ch1(int16_t *zeropoint1,int16_t *zeropoint2)
{
	for(int i=1200;i<2000;i++)
	{
		...
		if(i==1999)*zeropoint1=0;
	}
	for(int i=*zeropoint1+5;i<2000;i++)
	{
		if(ad1[i]<=Trigger_set+Trigger_set_offset)
		{
			if(ad1[i+2]>Trigger_set+Trigger_set_offset) 
...
void findzeropoint_ch2(int16_t *zeropoint1,int16_t *zeropoint2)
{
	for(int i=1200;i<2000;i++)
	{
		...
		if(i==1999)*zeropoint1=*zeropoint2=0;
	}
	for(int i=*zeropoint1+5;i<2000;i++)
	{
		if(ad2[i]<=Trigger_set+Trigger_set_offset)
...
{
	int16_t average_vol1,average_vol2;
	...
	average_vol1/=200;
	average_vol2/=200;
	*calbo1+=128-average_vol1;
	*calbo2+=128-average_vol2;
}