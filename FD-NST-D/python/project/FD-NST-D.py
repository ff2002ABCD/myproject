...
class CalibrationChartWindow(QDialog):
        ...
        for i in range(7):
            ...
            try:
                ...
                if voltage_mV != 0:
                    ...
                    data.append((mass_g, force_N, voltage_mV))
            except ValueError:
                pass
        
        # 获取拟合参数
        sensitivity = self.sensitivity_label.text()
        correlation = self.correlation_label.text()
        gravity = self.gravity_edit.text()
    ...
    # 添加方法获取拟合曲线数据
    def get_fitted_curve(self, num_points=20):
        """获取拟合曲线数据"""
        # 获取当前显示的范围
        ...
        # 获取拟合参数
        try:
            slope = float(self.sensitivity_label.text())
            # 从图表中获取截距（需要从series中提取）
            points = []
...
class SurfaceTensionApp(QMainWindow):
    ...
    def init_left_area(self):
       ...
       # 左侧显示框
        ...
        self.nav_layout.setRowStretch(0, 1)    # 第一行可`拉伸
