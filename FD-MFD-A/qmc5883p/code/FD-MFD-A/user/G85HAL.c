...
{
		if(HAL_I2C_IsDeviceReady(hi2c1, QMC5883P_Addr, 3, 100)!=HAL_OK) return;
 		unsigned char cdata[3]={0x06,0x08,0XCD};
		HAL_I2C_Mem_Write(hi2c1,QMC5883P_Addr,0x29, I2C_MEMADD_SIZE_8BIT,cdata, 1, 100);          
		HAL_I2C_Mem_Write(hi2c1,QMC5883P_Addr,0x0B, I2C_MEMADD_SIZE_8BIT, cdata+1, 1, 100);            
		HAL_I2C_Mem_Write(hi2c1,QMC5883P_Addr,0x0A, I2C_MEMADD_SIZE_8BIT, cdata+2, 1, 100);     
}
...
void QMC5883P_rawread(float *GaX, float *GaY,float *GaZ)
{
		...
		if(HAL_I2C_IsDeviceReady(&hi2c1, QMC5883P_Addr, 3, 100)!=HAL_OK) return;
//	if(HAL_I2C_Mem_Read(&hi2c1, QMC5883P_Addr, 0x09,I2C_MEMADD_SIZE_8BIT, &status, 1, 100) != HAL_OK) return;
//	if(!(status & 0x01)) return;
//	uint8_t status;
//do {
//    HAL_I2C_Mem_Read(&hi2c1, QMC5883P_Addr, HMC5883_REG_STATUS, I2C_MEMADD_SIZE_8BIT, &status, 1, 100);
//} while(!(status & 0x01));
//	if(status & 0x02) return;
		uint8_t data[6];
		if(HAL_I2C_Mem_Read(&hi2c1,QMC5883P_Addr, 0x01,1,data,6 , 100)!=HAL_OK)return ; //连续读取
		int16_t dxra,dyra,dzra;
		dxra = (data[3] << 8) | data[2];
//		if(dxra>3000||dxra<-3000) return;
		*GaX = (float)dxra /3750.0f;	
		dyra = (data[1] << 8) | data[0]; 
//		if(dyra>3000||dyra<-3000) return;
		*GaY = (float)dyra /3750.0f ;	
	  dzra = (data[5] << 8) | data[4];	
//		if(dzra>3000||dzra<-3000) return;
		*GaZ = (float)dzra /3750.0f ;		
