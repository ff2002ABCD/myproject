...
_Bool input_mode;//0手动 1自动
_Bool input_judge;//0错误 1正确
...
void up_button()
{
	switch(page)
	{
		...
		case SELECT_INPUTMODE://选择输入方式 上
		...
		case GENERATE_COORDINATES://坐标生成 上
		...
		case OUTPUT_COMPLETE://导出完成 上
			printf("page 数据查询和导出\xff\xff\xff");
			page=DATA_OUTPUT;
		break;
	}
}
...
void confirm_button()
{
	switch(page)
	{
		...
		case MAIN://主页 确定
			...
			{
				case CALIBO:
					printf("page 向上固定\xff\xff\xff");
					page=PLACE_UP;
					
				break;
					
				case MEA_GND:
					printf("page 地磁场测量\xff\xff\xff");
					page=MEASURE_GROUND;
				break;
				
				case MEA_COIL:
					printf("page 自动置零\xff\xff\xff");
					page=AUTO_SETZERO;
				break;
		...
		//confirm
		case PLACE_UP:
			printf("page 校准XY轴零点\xff\xff\xff");
			page=XY_CALIBO;
			calibo_state=X0Y0;
			break;
		case XY_CALIBO:
			printf("page 向右固定\xff\xff\xff");
			page=PLACE_RIGHT;
			break;
		break;
		case PLACE_RIGHT:
			printf("page 校准Z轴零点\xff\xff\xff");
			page=Z_CALIBO;
			calibo_state=Z0;
			break;
		case Z_CALIBO:
			page=PLACE;
			printf("page 放置器件\xff\xff\xff");
			calibo_state=NONE;
		break;
		
		case PLACE:
			page=XY_CALIBO_1;
			printf("page 校准XY轴系数\xff\xff\xff");
			calibo_state=X1Y1;
		break;
		...
		case PLACE_RIGHT_1:
			page=Z_CALIBO_1;
			printf("page 校准Z轴系数\xff\xff\xff");
			calibo_state=Z1;
			break;
		case Z_CALIBO_1:
			page=CALIBO_SUCCESS;
			printf("page 校准完成\xff\xff\xff");
			calibo_state=NONE;
		break;
		...
		case AUTO_SETZERO:
			printf("page 线圈磁场测量\xff\xff\xff");
			Current=100;
			page=MEASURE_COIL;
			cursor_measure_coil=DATA_RECORD;
		...
		case MEASURE_COIL:
			...
			{
				printf("page 输入方式选择\xff\xff\xff");
			...
			if(input_mode==0)
			{
				printf("page 第XX组数据采集\xff\xff\xff");
				page=MEASUER_START;
				cursor_measuer_start=P1;
				if(group_num<16) group_now=group_num;
				else {group_now=0;group_num=0;}
				number_now=0;
			}
			else
			{
				printf("page 辅助生成坐标\xff\xff\xff");
				if(group_num<16)group_now=group_num;
				else {group_now=0;group_num=0;}
				number_now=0;
		...
		case GENERATE_COORDINATES:
			...
			else
			{
				...
				printf("page 辅助数据采集\xff\xff\xff");
			}
		break;
...
//			for(int i=0;i<16;i++)
//			{
//				printf("t%d.txt=\"第%d组\"\xff\xff\xff",i,cursor_sheet_num*16+i);
//			}
			printf("t%d.bco=61277\xff\xff\xff",cursor_group_num);
		break;
		//down
...
//	}
	...
	{
		if(beishu==1)
		{beishu=10;printf("move p0,257,157,257,157,0,0\xff\xff\xff");}
		else if(beishu==10){beishu=100;printf("move p0,236,157,236,157,0,0\xff\xff\xff");}
		else if(beishu==100){beishu=1;printf("move p0,288,157,288,157,0,0\xff\xff\xff");}
	}
