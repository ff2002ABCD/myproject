...
#define SCL(x)		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, x)
#define SDA(x)		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, x)
...
int HMC5883_RecvACK()
{
    ...
    SCL(1);                    //拉高时钟线
    Delay5us();                 //延时
    CY = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_11);                   //读应答信号
    SCL(0);                    //拉低时钟线
    Delay5us();                 //延时
...
BYTE HMC5883_RecvByte()
{
    ...
    for (i=0; i<8; i++)         //8位计数器
    {
        ...
        SCL(1);                //拉高时钟线
        Delay5us();             //延时
        dat |= HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_11);               //读数据               
					SCL(0);                //拉低时钟线
        Delay5us();             //延时
    }
    return dat;
