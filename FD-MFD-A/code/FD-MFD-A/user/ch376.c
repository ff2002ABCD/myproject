...
//	if(xReadCH376Data()==USB_INT_DISK_WRITE) {}
//		else return 0;
//		//printf("正在写入数据...");
...
void ch376_init()
{
	...
	while(retry--) 
	{
		...
		if(xReadCH376Data()==0x51)
		{
			...
			{
				printf("设置成功\xff\xff\xff");
				break;
			}
			else{
				printf("设置失败\xff\xff\xff");
		   }
			if(retry==0) 
			{
				init_flag376=0;
	...
	while(retry--)
	{
		...
		else {printf("连接错误\xff\xff\xff");}
		if(retry==0) 
			{
				printf("连接失败\xff\xff\xff");
				return;
			}
		HAL_Delay(500);
	}
...
void ch376_writetest()
{
	...
	while(retry--)
	{
		...
		{
			printf("打开错误\xff\xff\xff");
		}
		if(retry==0) 
		{
			printf("打开失败\xff\xff\xff");
			return;
		}
		HAL_Delay(500);
...
//	printf("size=%d\r\n",size);
	printf("正在写入表头...\xff\xff\xff");
	//写入表头
	if(file_writeData("序号,传感器坐标(cm),线圈电流(mA),磁感应强度模(uT),X轴分量(uT),Y轴分量(uT),Z轴分量(uT)\n")
		==USB_INT_SUCCESS) 
	{
		printf("表头写入成功\xff\xff\xff");
	}
	else 
	{
		
		printf("表头写入失败\xff\xff\xff");
		return;
	}
...
//	else printf("写入失败\xff\xff\xff");
	...
	for(int i=0;i<table_length[cursor_group_num];i++)
	{
		...
		table[cursor_group_num][i].Pos,table[cursor_group_num][i].Current,
		table[cursor_group_num][i].Strength,table[cursor_group_num][i].GaX,
		table[cursor_group_num][i].GaY,table[cursor_group_num][i].GaZ);
...
//	printf("filesize=%d\xff\xff\xff",size);
	...
	while(retry--){
		...
		{
			printf("关闭错误\xff\xff\xff");
		}
		if(retry==0) 
		{
			printf("关闭失败\xff\xff\xff");
			return;
		}
		HAL_Delay(500);
