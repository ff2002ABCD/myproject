...
uint16_t index;
uint16_t num=1000;//²¨ÐÎ·Ö±æÂÊ
volatile _Bool adc_start=0;
...
void square_handle()
{
		...
		if(direction_counter<duty_cycle-1) dac_value=dac_reverse_max;
		else dac_value=dac_reverse_min;
		direction_counter++;
		if(direction_counter>=duty_cycle) 
		{
			direction_counter=0;
			adc_start=1;
		}
		
		if(direct_current_output==1) dac_value=dac_reverse_min;
}
