        ...
        case SPECTRUM_MEASURE:
            ...
            {   
                ...
                case CURSOR_CALIBRATION:
                    ...
                    printf("yanshi.bco=61277\xff\xff\xff");
                    cursor_spectrum = CURSOR_YANSHI;
                    break;
                case CURSOR_YANSHI:
                    printf("yanshi.bco=50779\xff\xff\xff");
                    printf("reset2.bco=61277\xff\xff\xff");
                    cursor_spectrum = CURSOR_RESET_ZOOM;
                    break;  
                case CURSOR_ZOOM:
        ...
        case SPECTRUM_MEASURE:
            ...
            switch(cursor_spectrum) {
                ...
                case CURSOR_YANSHI:
                {
                    ...
                    // 【修复】声明 ProcessedDataBuffer 外部变量
                    extern uint8_t ProcessedDataBuffer[];  // 自动缩放函数使用的缓冲区
                    
                    // 将演示数据复制到RAM
                    memcpy(spectrum_data_ram[0], demo_spectrum_data, DEMO_DATA_SIZE);
                    
                    // 【修复】同时将演示数据填充到 ProcessedDataBuffer（729个点）
                    // ProcessedDataBuffer 用于自动缩放计算，需要正确填充
                    for(int i = 0; i < 729; i++) {
                        int src_index = i * 5;  // 3648/729 ≈ 5
                        if(src_index < DATA_BUFFER_SIZE) {
                            ProcessedDataBuffer[i] = demo_spectrum_data[src_index];
                        } else {
                            ProcessedDataBuffer[i] = 0;  // 防止越界
                        }
                    }
                    
                    // 标记数据有效
                    spectrum_groups_info[0].is_valid = 1;
                    spectrum_groups_info[0].data_length = DATA_BUFFER_SIZE;
                    
                    // 【设置横坐标为标定后的波长值】
                    // 演示数据的标定参数：k=0.083478, b=324.52（根据DATA.csv计算）
                    // 像素0对应324.60nm，像素3647对应629.15nm
                    float demo_k = 0.083478f;
                    float demo_b = 324.52f;
                    
                    // 更新横坐标显示为波长
                    printf("vis z1,0\xff\xff\xff");    // 隐藏像素坐标
                    printf("vis z2,1\xff\xff\xff");    // 显示波长坐标
                    
                    // 设置t8-t15的横坐标为波长值
                    for(int i = 0; i < 8; i++) {
                        int pixel_val = DEFAULT_T_VALUES[i]; // 0, 500, 1000, 1500, 2000, 2500, 3000, 3500
                        float wavelength = demo_k * pixel_val + demo_b;
                        printf("t%d.txt=\"%.1f\"\xff\xff\xff", i + 8, wavelength);
                        display_range.current_values[i] = DEFAULT_T_VALUES[i]; // 存储像素值
                    }
                    ...
                    // 显示调试信息
                    printf("debug.txt=\"演示数据已加载\"\xff\xff\xff");
                    break;
                }
                case CURSOR_CONT_COLLECT:
                {
        ...
        case CALIBRATION:
            ...
            switch(cursor_calibration1) {
                ...
                case CURSOR_INPUT_COEFF:
                    // 输入系数 - 从CALIBO.TXT文件中读取校准参数
                    printf("正在初始化U盘...\xff\xff\xff");
                    
                    // 使用统一的CH376初始化函数（带超时保护）
                    if(ch376_init_with_baudrate_change()) {
                        ...
                        if(ch376_read_calibration()) {
                            ...
                            // 设置t8的波长显示
                            int t8_pixel = DEFAULT_T_VALUES[0];
        ...
        case QUERY_EXPORT:
            ...
            switch(cursor_spectrum) {
                ...
                if(!ch376_init_with_baudrate_change()) {
                    // 初始化失败，直接显示失败页面
                    printf("page outed\xff\xff\xff");
                    for(volatile uint32_t delay_count = 0; delay_count < 2000000; delay_count++) { __NOP(); }  // 等待页面切换完成(200ms)
                    page=OUTPUT_COMPLETE;
