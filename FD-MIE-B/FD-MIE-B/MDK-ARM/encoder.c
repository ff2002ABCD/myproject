...
void clockwise()
{
	...
	switch(page)
	{
		...
		case QUERY_JOURNEY:
		{	
			if(x_offset_journey+x_offset_move>=(count_max-count[index_save-1])/xgrid_multiple_journey[xgrid_multiple_journey_index])
			{
				x_offset_journey=(count_max-count[index_save-1])/xgrid_multiple_journey[xgrid_multiple_journey_index];
			}
			else x_offset_journey+=x_offset_move;
...
//			}
		...
		case QUERY_ANGLE:
		{
			if(x_offset_angle+x_offset_move>=(angle_max-angle[index_save_angle-1])/xgrid_multiple_angle[xgrid_multiple_angle_index])
			{
				x_offset_angle=(angle_max-angle[index_save_angle-1])/xgrid_multiple_angle[xgrid_multiple_angle_index];
			}
			else x_offset_angle+=x_offset_move;
			renew_screen_flag=1;
		}break;
		case QUERY_TIME:
		{
...
void counter_clockwise()
{
	...
	switch(page)
	{
		...
		case QUERY_JOURNEY:
		{	
			if(x_offset_journey-x_offset_move<=(count_min-count[index_save-1])/xgrid_multiple_journey[xgrid_multiple_journey_index])
			{
...
//			}
		...
		case QUERY_ANGLE:
		{
			if(x_offset_angle-x_offset_move<=(angle_min-angle[index_save_angle-1])/xgrid_multiple_angle[xgrid_multiple_angle_index])
			{
