...
class HeatingExperiment:
    ...
    def create_widgets(self):
        ...
        self.air_button = ttk.Button(self.control_frame, text="放置空气样品", 
    ...
    def toggle_air(self):
        ...
        if self.air_position == "取下":
            # 检查是否可以放置空气样品（橡胶样品不能在中区域）
            if self.rubber_position != "中":
                self.air_position = "中"
                self.show_air_in_middle()
                self.middle_frame.config(height=14)  # 高度减小2/3
                self.top_frame.config(height=150)  # 高度减小2/3
                self.air_button.config(text="取下空气样品")
                self.rubber_button.config(state=tk.DISABLED)  # 禁用橡胶样品按钮
                # 自动调整加热器位置
                self.auto_adjust_heater_position()
            else:
                # 如果橡胶样品在中区域，不能放置空气样品
                tk.messagebox.showwarning("操作失败", "橡胶样品在中区域时不能放置空气样品！")
        else:
            self.air_position = "取下"
            
            self.hide_air()
            self.middle_frame.config(height=14)
            self.air_button.config(text="放置空气样品")
    ...
    def update_interface(self):
        ...
        self.air_button.config(text="取下空气样品" if self.air_position == "中" else "放置空气样品")
