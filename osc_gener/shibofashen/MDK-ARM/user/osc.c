...
_Bool ready=0;
//菜单1相关的变量
uint8_t menu_status=9,Ouhe=0,Ouhe1=0,Channel=0,Trigger_state=1;
...
int flag,flag1,flag2,flag3,flag4;//传输完成标志
uint16_t mem2[S*K];
uint16_t buffer1[S*K/4],buffer2[S*K/4],buffer3[S*K/4],buffer4[S*K/4];
int32_t temp,temp0;
int ADC_flag;
uint8_t RX_buffer[16];

int16_t offset=0;
int16_t offset_ch1=100;
int16_t offset_ch2=-50;
...
//		case _10mV:
	...
	voltage_per_grid_1=light_scale_now;
...
void Reconstruct_Data(void)
{
		memset(mem2,0,S);
    // ? 注意：别人只取高8位，说明有效数据在高字节
    for(int i = 0; i < S/4; i++)
    {
        mem2[i * 4 + 0] = buffer1[i] ;   // 取高8位
        mem2[i * 4 + 1] = buffer2[i] ;
       mem2[i * 4 + 2] = buffer3[i] ;
        mem2[i * 4 + 3] = buffer4[i] ;
    }
}
...
//    for(int i = 0; i < S/8; i++)
//    {
//      mem2[i * 8 + 0] = buffer1[i]& 0xFF ;        // 取低8位
//			mem2[i * 8 + 4] = (buffer1[i]>>8)& 0xFF ;   // 取高8位
//      mem2[i * 8 + 1] = buffer2[i]& 0xFF ;   
//			mem2[i * 8 + 5] = (buffer2[i]>>8)& 0xFF ;   
//			mem2[i * 8 + 2] = buffer3[i]& 0xFF ;   
//			mem2[i * 8 + 6] = (buffer3[i]>>8)& 0xFF ;   
//			mem2[i * 8 + 3] = buffer4[i]& 0xFF ;   
//			mem2[i * 8 + 7] = (buffer4[i]>>8)& 0xFF ;  
//			
//    }
//}
...
{	
  ...
  if(htim->Instance==htim3.Instance)
	...
	//	HAL_UART_Receive_IT(&huart4, RX_buffer, 4);
		...
		if(tim3_counter_1>=200) //10K/200=50Hz
		{
			tim3_counter_1=0;
			if(osc_state==FREE)
			{
		...
		if(tim3_counter>=200) //10K/200=50Hz
		{
			tim3_counter=0;
	...
	if (huart->Instance == UART4) {
		...
		//	printf("透传结束\xff\xff\xff");
			...
			else if(osc_state==CH2_SENDING)
			{
				...
				if(time_grid_now+1==_500ns)
				{
					TIM2->PSC=1-1;
					Start_Sampling_Fixed();
				}
				else if(time_grid_now+1==_1us)
				{
					TIM2->PSC=2-1;
					Start_Sampling_Fixed();
				}
...
void Start_Sampling_Fixed(void)
{
    // 1. 停止所有
    ...
    // 4. 清除所有标志
    TIM2->SR = 0;
    DMA1->LIFCR = 0x3F << 6;   // 清除 Stream2
    DMA2->LIFCR = 0x3F;        // 清除 Stream0
    DMA1->LIFCR = 0x3F << 12;  // 清除 Stream3
    DMA2->LIFCR = 0x3F << 6;   // 清除 Stream1
    
		
    // 5. 启动 DMA
    HAL_DMA_Start_IT(&hdma_tim2_ch1, (uint32_t)&GPIOD->IDR, 
                     (uint32_t)buffer1, S*K/4);
    HAL_DMA_Start_IT(&hdma_tim2_ch2, (uint32_t)&GPIOD->IDR, 
                     (uint32_t)buffer2, S*K/4);
    HAL_DMA_Start_IT(&hdma_tim2_ch3, (uint32_t)&GPIOD->IDR, 
                     (uint32_t)buffer3, S*K/4);
    HAL_DMA_Start_IT(&hdma_tim2_ch4, (uint32_t)&GPIOD->IDR, 
                     (uint32_t)buffer4, S*K/4);
