...
class ChaosInterface(ttk.Frame):
    ...
    def setup_right_frame(self):
        ...
        # 放上电感按钮
        self.inductor_on_btn = ttk.Button(inductor_btn_frame, text="放上电感", 
        ...
        # W1滑块
        self.w1_slider = tk.Scale(w1_horizontal_frame, from_=1.685, to=1.80, 
        ...
        # W2滑块
        self.w2_slider = tk.Scale(w2_horizontal_frame, from_=-0.1, to=0.1, 
...
class IVInterface(ttk.Frame):
    ...
    def update_resistance_display(self):
        """更新电阻显示和对应的电压"""
        # 格式化电阻显示：5位整数 + 1位小数
        # integer_part = int(self.resistance_value)
        # decimal_part = int(round((self.resistance_value - integer_part) * 10))
        
        # resistance_str = f"{integer_part:05d}.{decimal_part} Ω"
        # self.resistance_var.set(resistance_str)
        
        # 计算对应电压：电阻0 -> -12.6V, 电阻99999.9 -> -0.01V
        min_resistance = 0.0
        max_resistance = 99999.9
        min_voltage = -12.6  # 最小电阻对应最负的电压
        max_voltage = -0.01  # 最大电阻对应最接近0的电压
        ...
        # 线性插值
        if max_resistance - min_resistance != 0:
            voltage = min_voltage + (self.resistance_value - min_resistance) * (max_voltage - min_voltage) / (max_resistance - min_resistance)
        else:
            voltage = min_voltage
        
        # 更新电压显示
        voltage_label_text = f"对应电压: {voltage:.3f} V"
        # 查找电压标签并更新
        for child in self.right_frame.winfo_children():
            if isinstance(child, ttk.LabelFrame) and child['text'] == "电阻箱控制":
                for subchild in child.winfo_children():
                    if isinstance(subchild, ttk.Label) and subchild.cget('text').startswith("对应电压:"):
                        subchild.config(text=voltage_label_text)
    ...
    def refresh_table(self):
        ...
        for i, (v, i_mA) in enumerate(zip(self.measure_V, self.measure_I), 1):
            # 计算电阻
            if abs(i_mA) > 1e-10:
                resistance_ohm = abs(v / i_mA * 1000)
    ...
    def delete_selected_row(self):
        ...
        for item in selected:
            ...
            if values:
                ...
                # 查找并删除匹配的数据点
                for i in range(len(self.measure_V) - 1, -1, -1):
                    if abs(self.measure_V[i] - voltage) < 1e-6 and abs(self.measure_I[i] - current) < 1e-6:
                        self.measure_V.pop(i)
                        self.measure_I.pop(i)
                        break
    ...
    def update_scatter(self):
        """更新散点图（只更新散点，不更新连线）"""
        if self.measure_V and self.measure_I:
            self.measure_scatter.set_offsets(np.c_[self.measure_V, self.measure_I])
        else:
