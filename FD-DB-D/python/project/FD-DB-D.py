...
class PendulumSimulator:
    def __init__(self, root):
        ...
        self.saved_calculations = {
            "固定摆长改变摆角": {
                ...
                "current_length": L0  # 保存计算时的摆长
            },
            "改变摆长": {
                "g_value": "未计算", 
                "error": "未计算",
                "show_calculation": False,
                "current_length": None  # 改变摆长实验不需要保存固定摆长
            }
        }
        ...
        self.long_press_interval = 50  # 长按间隔（毫秒）
    ...
    def create_widgets(self):
        ...
        if os.path.exists(bg_image_path):
            try:
                pil_image = Image.open(bg_image_path)
                pil_image = pil_image.resize((800, 300), Image.Resampling.LANCZOS)
                self.bg_image = ImageTk.PhotoImage(pil_image)
                
                self.exp_canvas = tk.Canvas(right_frame, width=800, height=300)
                self.exp_canvas.pack()
                self.exp_canvas.create_image(0, 0, image=self.bg_image, anchor="nw")
                
                # 在最低点位置添加红点
                self.red_dot_id = self.exp_canvas.create_oval(520+70, 200+5, 550+70, 230+5, 
                                                            fill="red", outline="red", tags="red_dot")
            except Exception as e:
                print(f"加载背景图片失败: {e}")
                self.exp_canvas = tk.Canvas(right_frame, width=800, height=300, bg="lightgray")
                self.exp_canvas.pack()
            ...
            self.exp_canvas = tk.Canvas(right_frame, width=800, height=300, bg="lightgray")
            self.exp_canvas.pack()
            self.red_dot_id = self.exp_canvas.create_oval(470+90, 150, 490+90, 170, 
                                                        fill="red", outline="red", tags="red_dot")
        ...
        # 摆长微调按钮框架
        length_control_frame = ttk.Frame(control_frame)
        length_control_frame.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        
        # 摆长微调按钮
        self.length_minus_btn = ttk.Button(length_control_frame, text="-", width=3)
        self.length_minus_btn.pack(side=tk.LEFT)
        
        self.length_var = tk.DoubleVar(value=L0)
        self.length_scale = ttk.Scale(length_control_frame, from_=0.1, to=0.75, 
                                    variable=self.length_var, orient=tk.HORIZONTAL,
                                    command=self.on_length_change,
                                    length=230)  # 减少进度条长度以容纳按钮
        self.length_scale.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        self.length_plus_btn = ttk.Button(length_control_frame, text="+", width=3)
        self.length_plus_btn.pack(side=tk.LEFT)
        ...
        # 传感器位置微调按钮框架
        sensor_control_frame = ttk.Frame(control_frame)
        sensor_control_frame.grid(row=0, column=4, sticky=(tk.W, tk.E), padx=5)
        
        # 传感器位置微调按钮
        self.sensor_minus_btn = ttk.Button(sensor_control_frame, text="-", width=3)
        self.sensor_minus_btn.pack(side=tk.LEFT)
        
        self.sensor_var = tk.DoubleVar(value=self.sensor_position)
        self.sensor_scale = ttk.Scale(sensor_control_frame, from_=L0+0.012, to=0.75, 
                                    variable=self.sensor_var, orient=tk.HORIZONTAL,
                                    command=self.on_sensor_change,
                                    length=230)
        self.sensor_scale.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        self.sensor_plus_btn = ttk.Button(sensor_control_frame, text="+", width=3)
        self.sensor_plus_btn.pack(side=tk.LEFT)
        ...
        # 摆角微调按钮框架
        angle_control_frame = ttk.Frame(control_frame)
        angle_control_frame.grid(row=1, column=1, sticky=(tk.W, tk.E), padx=5)

        # 摆角微调按钮
        self.angle_minus_btn = ttk.Button(angle_control_frame, text="-", width=3)
        self.angle_minus_btn.pack(side=tk.LEFT)

        self.angle_scale = ttk.Scale(angle_control_frame, from_=-50, to=50,
        ...
        self.angle_plus_btn = ttk.Button(angle_control_frame, text="+", width=3)
        self.angle_plus_btn.pack(side=tk.LEFT)
        ...
        self.reset_btn = tk.Button(self.exp_canvas, text="", width=3,height=1, command=self.on_reset_click,
                                ...
                                activebackground=bg_color, activeforeground="blue")
        self.reset_btn.place(x=393+57, y=205)
        ...
        # 绑定微调按钮的长按功能
        self.bind_long_press_buttons()
    
    def bind_long_press_buttons(self):
        """绑定所有微调按钮的长按功能"""
        # 摆长按钮
        self.length_minus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(-0.001, 'length'))
        self.length_minus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())
        
        self.length_plus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(0.001, 'length'))
        self.length_plus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())
        
        # 传感器位置按钮
        self.sensor_minus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(-0.001, 'sensor'))
        self.sensor_minus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())
        
        self.sensor_plus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(0.001, 'sensor'))
        self.sensor_plus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())
        
        # 摆角按钮
        self.angle_minus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(-1, 'angle'))
        self.angle_minus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())
        
        self.angle_plus_btn.bind('<ButtonPress-1>', lambda e: self.start_long_press(1, 'angle'))
        self.angle_plus_btn.bind('<ButtonRelease-1>', lambda e: self.stop_long_press())

    def start_long_press(self, delta, control_type):
        """开始长按"""
        self.long_press_delta = delta
        self.long_press_control_type = control_type
        
        # 立即执行一次调整
        self.execute_adjustment()
        
        # 设置长按重复
        self.after_id = self.root.after(self.long_press_delay, self.repeat_long_press)

    def repeat_long_press(self):
        """重复长按操作"""
        if hasattr(self, 'long_press_delta'):
            self.execute_adjustment()
            self.after_id = self.root.after(self.long_press_interval, self.repeat_long_press)
    ...
    def execute_adjustment(self):
        """执行调整操作"""
        if hasattr(self, 'long_press_delta') and hasattr(self, 'long_press_control_type'):
            delta = self.long_press_delta
            control_type = self.long_press_control_type
            
            if control_type == 'length':
                self.adjust_length(delta)
            elif control_type == 'sensor':
                self.adjust_sensor(delta)
    ...
    def adjust_length(self, delta):
        ...
        # 确保新值在有效范围内
        if 0.1 <= new_value <= 0.75:
            self.length_var.set(new_value)
            self.on_length_change(new_value)
    
    def adjust_sensor(self, delta):
        """调整霍尔传感器位置"""
        current_value = self.sensor_var.get()
        new_value = current_value + delta
        
        # 确保新值在有效范围内
        min_sensor = self.current_L + 0.012
        if min_sensor <= new_value <= 0.75:
            self.sensor_var.set(new_value)
            self.on_sensor_change(new_value)

    def adjust_angle(self, delta):
        """调整摆角"""
        current_value = self.angle_var.get()
        new_value = current_value + delta

        # 确保新值在有效范围内
        if -50 <= new_value <= 50:
            self.angle_var.set(new_value)
            self.on_angle_change(new_value)
    ...
    def calculate_gravity_acceleration(self):
        """计算重力加速度和误差（只在点击计算按钮时调用）"""
        if self.data_table is None or not self.data_table.get_children():
            self.g_value_var.set("数据不足")
            self.error_var.set("数据不足")
            # 保存结果
            self.saved_calculations[self.experiment_type]["g_value"] = "数据不足"
            self.saved_calculations[self.experiment_type]["error"] = "数据不足"
            return
        ...
        try:
            ...
            # 保存结果
            self.saved_calculations[self.experiment_type]["g_value"] = f"{g_calculated:.4f} m/s²"
            self.saved_calculations[self.experiment_type]["error"] = f"{percent_error:.2f}%"
            ...
            # 保存结果
            self.saved_calculations[self.experiment_type]["g_value"] = "计算错误"
            self.saved_calculations[self.experiment_type]["error"] = "计算错误"
    ...
    def update_chart_display_only(self):
        """只更新图表显示，不重新计算重力加速度"""
        self.chart_ax.clear()
        ...
        if self.experiment_type == "固定摆长改变摆角":
            ...
            self.update_chart_fixed_display_only()
        else:
            self.chart_ax.set_xlabel("L/m")
            self.chart_ax.set_ylabel("T^2/s^2")
            self.chart_ax.set_title("改变摆长L测量周期T的实验数据")
            self.update_chart_variable_display_only()
        
        self.chart_ax.grid(True)
        self.chart_canvas.draw()

    def update_chart_fixed_display_only(self):
        """只显示固定摆长图表，不进行计算"""
        # 收集数据
        x_data = []
        y_data = []
        ...
        # 构建标题
        title = "摆长不变时角度θ与2T的测量数据"
        if self.show_calculation and self.saved_calculations["固定摆长改变摆角"]["current_length"] is not None:
            current_length = self.saved_calculations["固定摆长改变摆角"]["current_length"]
            title += f"（当前摆长：{current_length:.3f}m）"
    ...
    def update_chart_variable_display_only(self):
        """只显示改变摆长图表，不进行计算"""
        # 收集数据
        x_data = []
        y_data = []
    ...
    def on_experiment_type_change(self):
        ...
        # 保存当前计算结果到旧类型
        self.saved_calculations[old_type]["g_value"] = self.g_value_var.get()
        self.saved_calculations[old_type]["error"] = self.error_var.get()
        self.saved_calculations[old_type]["show_calculation"] = self.show_calculation
        ...
        # 恢复新类型的计算结果
        self.g_value_var.set(self.saved_calculations[new_type]["g_value"])
        self.error_var.set(self.saved_calculations[new_type]["error"])
        self.show_calculation = self.saved_calculations[new_type]["show_calculation"]
        
        # 更新图表（但不重新计算重力加速度）
        self.update_chart_display_only()
    ...
    def clear_calculation_results(self):
        ...
        self.saved_calculations[self.experiment_type]["show_calculation"] = False
        self.saved_calculations[self.experiment_type]["g_value"] = "未计算"
        self.saved_calculations[self.experiment_type]["error"] = "未计算"
        
        # 如果是固定摆长实验，也重置摆长信息
        if self.experiment_type == "固定摆长改变摆角":
            self.saved_calculations[self.experiment_type]["current_length"] = None
    ...
    def on_table_click(self, event):
        ...
        # 只有当没有实际计算结果时才重置显示
        if self.saved_calculations[self.experiment_type]["g_value"] == "未计算":
            self.g_value_var.set("点击'计算'按钮显示结果")
            self.error_var.set("点击'计算'按钮显示结果")
    ...
    def update_sensor_position(self, L):
        sensor_length = 0.05
        sensor_y = -self.sensor_position-0.012  # 使用独立的传感器位置
        sensor_x_center = 0
    ...
    def on_length_change(self, val):
        global theta, omega, x, y
        # if self.is_animating or self.exp_mode == "查阅模式":
        #     # 在动画中或查阅模式下，不允许调整摆长
        ...
        if L0+0.01 > self.sensor_position:
            self.sensor_position = L0+0.012
            self.sensor_var.set(L0+0.012)
            self.sensor_label.config(text=f"{L0+0.012:.3f} m")
        
        self.sensor_scale.config(from_=L0+0.012)  # 设置传感器滑块的最小值为当前摆长
        
        self.update_sensor_position(L0+0.012)
    ...
    def reset_to_initial_angle(self):
        ...
        # 重新计算运动轨迹（使用当前设定的摆角）
        current_angle_deg = self.angle_var.get()
        theta0 = np.radians(current_angle_deg)
    ...
    def animate(self):
        ...
        if self.current_frame < len(t) - 1:
            self.current_frame += 2
            self.elapsed_time = self.current_frame * dt
            
            self.update_display()
            
            current_theta = theta[self.current_frame]
            # 检测是否在最低点
            at_bottom = self.is_at_bottom(y[self.current_frame], self.current_L, current_theta)
            
            # 只有在离开最低点后再次到达时才记录
            if at_bottom and not self.was_at_bottom and self.max_count>=1:
                if self.exp_mode == "设定模式":
                    # 首次到达最低点，切换到查阅模式，记录开始时间
                    self.exp_mode = "查阅模式"
                    self.query_start_time = self.elapsed_time  # 记录查阅模式开始时间
                    print("elaspsed time: ", self.elapsed_time)
                    self.current_count = 0
                    self.time_records = []
                    self.current_record_index = 0
                    self.mode_label.config(text="模式: 查阅模式")
                    
                    # 更新显示为0
                    self.count_label.config(text="0")
                    self.time_label.config(text="0.000")
                    
                    print("切换到查阅模式，准备开始记录")
                
                elif self.exp_mode == "查阅模式":
                    if self.max_count > 0 and self.current_count < self.max_count:
                        ...
                        self.current_record_index = self.current_count
            ...
            # # 更新红点颜色
            # self.update_red_dot_color(at_bottom)//在update_display中统一处理
            
            # 安排下一次更新
            self.root.after(int(dt * 1000), self.animate)
    
    def is_at_bottom(self, y_pos, L, current_theta):
        """检测是否到达最低点：y坐标接近传感器位置且角度接近0度"""
        # y坐标检测：使用传感器位置进行检测
        y_condition = abs(y_pos - (-self.sensor_position)) < 0.022
        
        # 角度检测：角度接近0度（考虑摆动方向）
        if np.degrees(theta0) <5:
            angle_condition = abs(current_theta) < 0.01  # 约±0.57度范围内
        else:
            angle_condition = abs(current_theta) < 0.06  # 约±3.44度范围内
    ...
    def update_display(self):
        ...
        current_theta = theta[self.current_frame]
        at_bottom = self.is_at_bottom(current_y, self.current_L,current_theta)
    ...
    def on_calculate_click(self):
        ...
        # 保存计算状态和当前摆长
        self.saved_calculations[self.experiment_type]["show_calculation"] = self.show_calculation
        if self.experiment_type == "固定摆长改变摆角":
            self.saved_calculations[self.experiment_type]["current_length"] = self.current_L
        
        self.update_chart()  # 更新图表，显示计算结果
        self.calculate_gravity_acceleration()  # 计算重力加速度
        # 计算后保存结果
        self.saved_calculations[self.experiment_type]["g_value"] = self.g_value_var.get()
        self.saved_calculations[self.experiment_type]["error"] = self.error_var.get()
    ...
    def on_plus_click(self):
        if self.exp_mode == "设定模式":
            ...
            if self.current_record_index < len(self.time_records) and self.current_record_index < self.current_count:
                ...
                self.count_label.config(text=f"{self.current_record_index}")
                self.time_label.config(text=f"{displayed_time:.3f}")
                
                print(f"查阅第 {self.current_record_index} 次，理论时间: {displayed_time:.3f}s")
    ...
    def on_minus_click(self):
        if self.exp_mode == "设定模式":
            ...
            if self.current_record_index > 1:
                ...
                print(f"查阅第 {self.current_record_index} 次，理论时间: {displayed_time:.3f}s (摆角: {np.degrees(theta0):.1f}°)")
