...
class PendulumSimulator:
    ...
    def on_length_change(self, val):
        ...
        self.sensor_scale.config(from_=L0+0.01)  # 设置传感器滑块的最小值为当前摆长
        ...
        if self.current_frame < len(t) - 1:
            ...
            if at_bottom and not self.was_at_bottom and self.max_count>=1:
                if self.exp_mode == "设定模式":
                    # 首次到达最低点，切换到查询模式，记录开始时间
                    self.exp_mode = "查询模式"
                    self.query_start_time = self.elapsed_time  # 记录查询模式开始时间
                    ...
                    print("切换到查询模式，准备开始记录")
                
                elif self.exp_mode == "查询模式":
                    # 在查询模式下记录时间和次数
                    if self.max_count > 0 and self.current_count < self.max_count:
                        self.current_count += 1
                        # 记录相对于查询模式开始的时间，精确到小数点后三位
                        actual_time = self.elapsed_time - self.query_start_time
                        # 添加千分之二的随机误差
                        displayed_time = self.add_random_error(actual_time)
                        # displayed_time = actual_time
                        self.time_records.append({
                            'actual': round(actual_time, 3),
                        ...
                        print(f"记录第 {self.current_count} 次到达最低点，实际时间: {actual_time:.3f}s, 显示时间: {displayed_time:.3f}s")
    ...
    def is_at_bottom(self, y_pos, L, current_theta):
        ...
        # y坐标检测：使用传感器位置进行检测
        y_condition = abs(y_pos - (-self.sensor_position)) < 0.012
        
        # 角度检测：角度接近0度（考虑摆动方向）
        angle_condition = abs(current_theta) < 0.02  # 约±2.86度范围内
